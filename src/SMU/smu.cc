...
// for each atom in `m`.
std::vector<int> ImplicitHydrogens(Molecule& m) {
...
void AddBond(Molecule& m, atom_number_t a1, atom_number_t a2,
             bond_type_t btype) {
...
bool FullyConnected(Molecule& m, atom_number_t zatom) {
...
// the two atoms.
void AddRings(Molecule& m, const SMU_Params& params,
  ...
  static const std::array<int, 4> bonds{NOT_A_BOND, SINGLE_BOND, DOUBLE_BOND,
                                        TRIPLE_BOND};
  ...
  for (int i = 0; i < matoms; ++i) {
    ...
    for (int j = i + 1; j < matoms; ++j) {
      ...
      for (int btype = 1; btype <= max_bonds; ++btype) {
        ...
        smu_results.Add(mcopy, already_produced,
                        params.non_aromatic_unique_smiles);
      }
    }
  }
}
...
void AddAtom(Molecule& m, const Atom& to_add) {
...
void ExpandFromAtom(Molecule& m, atom_number_t zatom, const SMU_Params& params,
  ...
  for (Atom* to_add : params.to_add) {
    ...
    for (int btype = 1; btype <= max_btype; ++btype) {
      ...
      //    cerr << "To " << mcopy->smiles() << " addinb bond btw " << zatom <<
      //    " and " << initial_natoms << " type " << bonds[btype] << "\n";
      AddBond(*mcopy, zatom, initial_natoms, bonds[btype]);
      // Because smu_results.Add may remove the pointer from mcopy on success,
      // save it here.
      Molecule* msave = mcopy.get();
      if (smu_results.Add(mcopy, already_produced,
                          params.non_aromatic_unique_smiles)) {
...
void ExpandMolecule(Molecule& m, const SMU_Params& params,
                    IW_STL_Hash_Set& already_produced,
                    SmuResults& smu_results) {
  const int matoms = m.natoms();
  for (int i = 0; i < matoms; ++i) {
