...
usage(int rc) {
  ...
  cerr << "  -D <file>      proto query file\n";
  cerr << "  -D F:<file>    file containing names of proto query files\n";
  cerr << "  -P ...        atom typing specification - determine changing atoms and searching "
          "match conditions (default=UST:AZUCORS)\n";
  ...
  // cerr << "  -K             aromatic bonds lose their Kekule forms - no longer match single or
  // double\n";
...
display_dash_M_options() {
  ...
  // cerr << "  -M aeuao       Atom Environments match Unmatched Atoms Only - unmatched during
  // matching\n";
  ...
  cerr << "  -M anmatch     array output will be the number of hits (rather than column per "
          "query)\n";
...
display_dash_j_options() {
  ...
  cerr << "  -j qmatch      isotopic label will be number of times any query hits an atom. No "
          "other -j possible\n";
...
static int
do_nhits_tag_output(Molecule& m, int nq, const int* hits, std::ostream& output) {
  if (!work_as_filter)
    write_smiles_and_pcn(m, output);
...
static int
do_array_output(const IWString& mname, int nhits, int nq, const int* hits, std::ostream& output) {
...
#ifdef DO_GETPSF
static int
do_getpsf(Molecule& m, const Substructure_Results& sresults) {
  int matoms = m.natoms();
  ...
  int* already_written = new_int(matoms);
  std::unique_ptr<int[]> free_already_written(already_written);
...
static int
write_bob_coner_special(const Substructure_Results* sresults, int nq, const IWString& mname) {
  if (mname.contains(' ')) {
    IWString tmp(mname);
    tmp.gsub(' ', '_');
...
apply_isotopic_labels_embedding(const Molecule& m,
  ...
  if (increment_isotopic_labels_to_indicate_queries_matching)
    return do_increment_isotopic_labels_to_indicate_queries_matching(m,
                                                                     *embedding,
                                                                     na,
                                                                     atom_isotopic_label);
  ...
  for (int j = 0; j < na; j++) {
    ...
    if (0 == atom_isotopic_label[k])  // definitely not already set
      ;
             ...
             negative_j_offset)  // we cannot distinguish an isotope in the initial molecule from
                                 // something added elsewhere here.
      ;
    else
...
label_matches_and_write_to_stream_for_individually_labelled_matches(
  ...
  for (int i = 0; i < sresults.number_embeddings(); i++) {
    ...
    if (!label_match_and_write_to_stream_for_individually_labelled_matches(m,
                                                                           query_number,
                                                                           sresults,
                                                                           i,
                                                                           atom_isotopic_label))
...
static int
transform_atoms_hit(const Substructure_Results& sresults, const Element** element_labels) {
  int ne = sresults.number_embeddings();
...
do_single_query(Molecule_to_Match& target,
  ...
  if ((verbose > 1 || report_matches) ||
      (0 == multiple_matches_each_query_counts_one && report_multiple_matches > 0 &&
       nmatches > report_multiple_matches))
    cerr << molecules_read << ": '" << target.molecule()->molecule_name() << "' " << nmatches
         << " matches to query '" << query->comment()
         << "' (total matches = " << query->molecules_which_match() << ")\n";

  if (0 == multiple_matches_each_query_counts_one && nmatches >= report_multiple_matches &&
      stream_for_multiple_matches.good())
...
do_all_queries(Molecule& m,
  ...
  for (int i = 0; i < nq; i++) {
    ...
    int nhits;
    if ((nhits = do_single_query(target,
                                 i,
                                 queries[i],
                                 sresults[i],
                                 element_labels,
...
tsubstructure(Molecule& m,
              ...
              int* queries_matched) {
  if (report_progress())
    cerr << "Processed " << molecules_read << " molecules, " << molecules_which_match
         << " molecules matched\n";
  ...
  if (nmatched) {
    ...
    if (write_matched_atoms_as_mdl_v30_atom_lists || write_matched_atoms_as_mdl_v30_bond_lists)
      write_matched_atoms_as_mdl_v30_lists(
          m,
          queries,
          sresults,
          stream_for_matching_structures.stream_for_type(FILE_TYPE_SDF));
...
tsubstructure(data_source_and_type<Molecule>& input,
  ...
  while (nullptr != (m = input.next_molecule())) {
    ...
    if (stop_processing_after_this_many_molecules_matching > 0 &&
        molecules_which_match >= stop_processing_after_this_many_molecules_matching) {
      if (verbose)
        cerr << "Processing halted because " << molecules_which_match
             << " molecules already matched\n";
...
handle_file_opening(Command_Line& cl,
  ...
  } else  // matched structures
  {
    for (int i = 0; i < queries.number_elements(); i++) {
      ...
      cerr << "Output stream for matches to query " << i << " '" << q->comment() << "' is '" << tmp
           << "'\n";
    }
  }
...
parse_set_of_options(Command_Line& cl,
  ...
  while (cl.value(flag, tmp, i)) {
    //  cerr << "processing -mn option '" << tmp << "' sw REPO = " << tmp.starts_with ("REPO") <<
    //  endl;
    ...
    if (tmp.starts_with("REPO")) {
      ...
      if (verbose)
        cerr << "Molecules which don't match the query will be written to the match stream (no "
                "QDT)\n";
    } else if ("alist" == tmp) {
      ...
      need_to_open_file = 1;
      if (verbose)
        cerr << match_or_non_match << "ed molecules assigned R(%d) numbers starting with " << j
             << endl;
    }
#endif
  ...
  return handle_file_opening(cl,
                             fname,
                             queries,
                             output_stream,
                             match_or_non_match,
                             separate_file_for_each_query);
}

static int
assign_name_if_needed(Substructure_Hit_Statistics& q, int query_number) {
...
tsubstructure(int argc, char** argv) {
  Command_Line cl(argc,
                  argv,
                  "w:W:y:R:X:hj:J:E:rcls:S:t:A:K:bBfm:n:uvo:i:q:Q:ag:pkG:Y:N:H:M:x:T:P:D:");
  ...
  if (cl.option_present('M')) {
    ...
    while (cl.value('M', m, i++)) {
      if ("impring" == m) {
        ...
        if (verbose)
          cerr << "Molecules containing non organics in the same fragment as the matched atoms "
                  "don't match\n";
      } else if ("tdt" == m || "TDT" == m) {
        work_as_filter = 1;
        ...
        m.remove_leading_chars(6);
        if (!m.numeric_value(stop_processing_after_this_many_molecules_matching) ||
            stop_processing_after_this_many_molecules_matching < 1) {
          cerr << "Invalid stop match criterion '" << m << "'\n";
          return 13;
        }

        if (verbose)
          cerr << "Will stop processing after "
               << stop_processing_after_this_many_molecules_matching
               << " molecules that match are found\n";
      } else if ("omlf" == m) {
        ...
        unsigned int r;
        if (!m.numeric_value(r) || r < 1) {
          cerr << "The report multiple matches value (rmm=) must be a whole +ve number '" << m
               << "' not recognised\n";
          return 2;
        }
    ...
    if (0 == max_atom_count)
      ...
      cerr << "Inconsistent min (" << min_atom_count << ") and max (" << max_atom_count
           << ") atom count specifications\n";
      return 4;
    } else if (verbose)
      cerr << "Will only match atoms with between " << min_atom_count << " and " << max_atom_count
           << " atoms\n";
  }
  ...
  if (cl.option_present('j')) {
    ...
    if ((positive_j_offset || negative_j_offset) && 0 == label_matched_atoms) {
      cerr << "Warning, positive (" << positive_j_offset << ") and/or negative ("
           << negative_j_offset
           << ") specified, but no label for non isotopic atoms. Any resulting negative isotopes "
              "ignored\n";
  ...
  if (cl.option_present('T')) {
    ...
    while (cl.value('T', fname, i++)) {
      if ("QDT" == fname) {
        ...
        if (verbose)
          cerr << "When looking for multiple matches each query counts once regardless of "
                  "embeddings\n";
      } else {
        if (!handle_file_opening(cl,
                                 fname,
                                 queries,
                                 stream_for_multiple_matches,
                                 "multiple matches",
                                 0))
          usage(7);
      }
  ...
  if (cl.option_present('J') && cl.option_present('a')) {
    cerr
        << "The -J (write as fingerprint) and -a (write as array) options are mutually exclusive\n";
    usage(27);
  }
  ...
  if (cl.option_present('J')) {
    ...
    tsubfp.set_default_fingerprint_nbits(default_fingerprint_nbits);
             ...
             array_output_is_number_of_matches)  //  Will we write Daylight fingerprints or an array
                                                 //  of hits
  {
    write_as_array = 1;
    if (verbose)
  ...
  int* queries_matched = new_int(nqueries + 1);
  std::unique_ptr<int[]> free_queries_matched(queries_matched);
  ...
  if (molecules_read > 0) {
    ...
    cerr << molecules_read << " molecules read, " << molecules_which_match
         << " molecules match fraction " << fraction << endl;
  }

  if (report_timing) {
    time_t execution_time = time(NULL) - tzero;

    cerr << "Took " << execution_time << " seconds, " << (molecules_read / execution_time)
  ...
  if (verbose && molecules_with_too_few_hits)
    cerr << molecules_with_too_few_hits << " molecules had fewer than " << min_hits_needed
         << " hits (-w option)\n";

  if (verbose && molecules_with_too_many_hits)
    cerr << molecules_with_too_many_hits << " molecules had more than " << max_hits_needed
         << " hits (-W option)\n";

  if (hits_in_non_organic_fragments_discarded)
    cerr << "Discarded " << hits_in_non_organic_fragments_discarded
         << " hits in non-organic fragments\n";
...
int
tsubstructure_csharp(int argc, char** argv) {
