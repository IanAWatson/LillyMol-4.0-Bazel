...
static int
identify_cf3_tbutyl (Molecule & m,
                     int * sp3)
{
  int rc = 0;
...
static int
sp3_filter (Molecule & m,
            IWString_and_File_Descriptor & output)
{
  int matoms = m.natoms();

  int * sp3 = new_int(matoms); std::unique_ptr<int[]> free_sp3(sp3);
...
static int
sp3_filter (data_source_and_type<Molecule> & input,
                IWString_and_File_Descriptor & output)
{
  Molecule * m;
...
sp3_filter (int argc, char ** argv)
{
  ...
  if (cl.option_present('c'))
  {
    ...
    if (verbose)
      cerr << "Molecules will be passed if they have at least " << min_carbon_sp3 << " sp3 Carbon atoms\n";
  }
  ...
  if (cl.option_present('x'))
  {
    ...
    {
      cerr << "The minimum number of non-Carbon sp3 atoms (-x) must be a whole +ve number\n";
      usage(2);
    }

    if (verbose)
      cerr << "Molecules will be passed if they have at least " << min_non_carbon_sp3 << " non Carbon sp3 atoms\n";
  }
