...
usage (int rc = 0)
{
  ...
  cerr << "Identifies substituents from substructure matched atoms\n";
...
class RGroup_Construction_Current_State
{
  ...
  public:
    ...
    int embedding_number () const { return _embedding_number;}
    int query_atom_number () const { return _query_atom_number;}

    void set_query_number(int s) { _query_number = s;}
    void set_embedding_number (int s) { _embedding_number = s;}
    void set_query_atom_number (int s) { _query_atom_number = s;}

    void set_parent_molecule_written(int s) { _parent_molecule_written = s;}
    ...
    int * atom_already_done () { return _atom_already_done;}
};
...
int
RGroup_Construction_Current_State::write_parent_molecule_if_needed (Molecule & m,
...
int
RGroup_Construction_Current_State::add_dummy_element_and_do_output (Molecule & r,
                                   int ndx,
                                   IWString_and_File_Descriptor & output) const
{
...
int
RGroup_Construction_Current_State::do_output (Molecule & r,
                                   int ndx,
                                   IWString_and_File_Descriptor & output) const
{

  output << r.unique_smiles() << " RG " << _query_number << '.' <<
            _embedding_number << '.' << _query_atom_number << '.' << ndx << '\n';
...
static int
build_molecule (const Molecule & m,
                atom_number_t a,
                bond_type_t   bt,
                ...
                Molecule & r)
{
  assert (0 == atom_already_done[a]);
  assert (a != anchor);

  int initial_natoms = r.natoms();

  atom_already_done[a] = initial_natoms + 1;
  ...
  for (int i = 0; i < acon; i++)
  {
    bond_type_t   bt;
    atom_number_t j;
    m.other_and_type (a, i, j, bt);

    if (j == anchor)
      continue;

#ifdef DEBUG_BUILD_MOLECULE
    cerr << "From atom " << a << " examine " << j << " already_done = " << atom_already_done[j] << 
            " contains = " << embedding.contains (j) << endl;
#endif
...
//  Be careful placing bonds, otherwise problems with cyclic structures
    ...
    if (atom_already_done[j])
    {
      if (atom_already_done[j] > initial_natoms)
        r.add_bond (initial_natoms, atom_already_done[j] - 1, bt);
      continue;
    }
    ...
    rc += build_molecule (m, j, bt, a, embedding, atom_already_done, r);
  }
...
static int
rgroup_including_substituents (Molecule & m,
                               RGroup_Construction_Current_State & rgrcs,
                               atom_number_t a,
                               const Set_of_Atoms & atoms_in_substituent,
                               const Set_of_Atoms & embedding,
                               IWString_and_File_Descriptor & output)
{
//cerr << "Processing embedding " << embedding << " from atom " << a << endl;
  ...
  for (int i = 0; i < atoms_in_substituent.number_elements(); i++)
  {
    ...
    build_molecule (m, j, NOT_A_BOND, INVALID_ATOM_NUMBER,
                      embedding, atom_already_done, r);
...
static int
identify_atoms_in_substituent (const Molecule & m,
                               atom_number_t ok_to_hit,
                               atom_number_t zatom,
                               int * atom_already_done)
{
#ifdef DEBUG_IDENTIFY_ATOMS_IN_SUBSTITUENT
  cerr << "identify_atoms_in_substituent:on to atom " << zatom << endl;
#endif
  ...
  for (int i = 0; i < acon; i++)
  {
    ...
    {
      if (zatom == ok_to_hit)    // coming off the attachment point, of course we hit other matched atoms
        continue;
...
{
  if (verbose > 1)
    cerr << "Trying to find ring R group from atom " << zatom << " ncon = " << m.ncon (zatom) << endl;
  ...
  if (0 == identify_atoms_in_substituent(m, zatom, zatom, atom_already_done))   // loops back
    return 1;

  if (verbose > 1)
    cerr << "Found OK substituent from " << zatom << endl;
                        ...
                        IWString_and_File_Descriptor & output)
{
  ...
  if (verbose > 1)
    cerr << "Trying to find R group from atom " << zatom << " ncon = " << m.ncon (zatom) << endl;

  int * atom_already_done = rgrcs.atom_already_done();

  set_vector(atom_already_done, m.natoms(), 0);

  Set_of_Atoms atoms_in_substituent;

  bool all_are_hydrogens = true;
  ...
  for (int i = 0; i < acon; i++)
  {
    ...
    if (verbose > 1)
      cerr << "From atom " << zatom << " atom " << j << " type " << m.smarts_equivalent_for_atom(j) <<  " already_done = " << atom_already_done[j] <<
              " in embedding = " << embedding.contains(j) << endl;

    if (atom_already_done[j])
    {
      cerr << "From atom " << zatom << " we find atom " << j << " already done. Possible cyclic\n";
      continue;
    }
    ...
    if (m.nrings(zatom) && m.nrings(j) && b->nrings())   // do not break up ring things
      continue;

    atoms_in_substituent.add(j);
    if (1 != m.atomic_number(j))
      all_are_hydrogens = false;
  }

  int n = atoms_in_substituent.number_elements();

  if (0 == n)    // nothing sprouting off here this time
    return 0;

//if (all_are_hydrogens && remove_explicit_hydrogens_not_attached_to_isotope

  if (n > 1 || m.hcount(zatom))
    return rgroup_including_substituents(m, rgrcs, zatom, atoms_in_substituent, embedding, output);

//cerr << "Normal processing atom atom " << zatom << endl;
  ...
  if (include_attachment_point_in_all_substituents)
  {
    ...
    build_molecule (m, atoms_in_substituent[0], NOT_A_BOND, INVALID_ATOM_NUMBER,
                    embedding, atom_already_done, r);
        ...
        IWString_and_File_Descriptor & output)
{
  ...
  if (verbose > 1)
    cerr << "Trying to find R group from atom " << a << " ncon = " << m.ncon (a) << endl;
  ...
  for (int i = 0; i < acon; i++)
  {
    ...
    if (verbose > 1)
      cerr << "From atom " << a << " atom " << j << " already_done = " << atom_already_done[j] <<
              " in embedding = " << embedding.contains (j) << endl;

    if (atom_already_done[j])
    {
      cerr << "From atom " << a << " we find atom " << j << " already done. Possible cyclic\n";
      continue;
    }
    ...
    if (m.in_same_ring(a, j))    // cannot process these
      continue;

    Molecule r;

    rc += build_molecule (m, j, NOT_A_BOND, INVALID_ATOM_NUMBER,
                          embedding, atom_already_done, r);
    ...
    rgrcs.add_dummy_element_and_do_output (r, i, output);
  }
                ...
                atom_number_t zatom)
{
  ...
  cerr << "is_within_ring:examining atom " << zatom << " acon " << acon << endl;
#endif
  ...
  for (int i = 0; i < acon; i++)
  {
    ...
    if (! s.contains(j))    // ring bond to something outside the embedding, bad
      return 1;
  }
...
rgroups (Molecule & m,
         RGroup_Construction_Current_State & rgrcs,
         const Set_of_Atoms & embedding,
         const Set_of_Query_Atoms * atoms,
         IWString_and_File_Descriptor & output)
{
  int rc = 0;
  ...
  for (int i = 0; i < na; i++)
  {
    ...
    cerr << "In query " << query_number << ", embedding " << embedding_number << 
            " item " << i << " query atom " << j << 
            " matches atom " << embedding->item (j) << endl;
#endif
    ...
    if (is_within_ring(m, embedding, embedding[j]))
    {
      ...
      rc += rgroup_combine_same_atom_substituents (m, rgrcs, embedding, embedding[j], output);
    else
...
static int
remove_embeddings_within_rings (Molecule & m,
         ...
         IWString_and_File_Descriptor & output)
{
  ...
  {
    if (verbose)
      cerr << "Only matched " << sresults.max_query_atoms_matched_in_search() << " query atoms when searching '" << m.name() << "'\n";
    return 0;
  }
  ...
  if (1 == nhits)
    ...
    cerr << m.molecule_name() << ", " << nhits << " hits to query '" << q->comment() << "'\n";
...
static int
rgroups (data_source_and_type<Molecule> & input,
         IWString_and_File_Descriptor & output)
{
  Molecule * m;
  while (nullptr != (m = input.next_molecule()))
  {
    ...
    if (verbose > 1)
      cerr << "Processing " << input.molecules_read() << " '" << m->molecule_name() << "'\n";
...
// Must be of the form Q.N
  ...
  if (! r.split(q, '.', n))
  {
    cerr << "Query atom specifications must be of the form 'Q.A' where Q is the query number and A is the atom\n";
    return 0;
  }
  ...
  {
    cerr << "Query " << query_number << " has " << aiq << " atoms only, so '" << n << "' is an invalid matched atom specifier\n";
    return 0;
  }
  ...
  if (verbose)
    cerr << "Will report R Groups from atom " << a << " in query " << query_number << endl;
  
  return 1;
}

static int
rgroups (const char * fname,
         FileType input_type,
         IWString_and_File_Descriptor & output)
{
...
rgroups (int argc, char **argv)
{
  ...
  if (cl.option_present ('D'))
  {
    ...
    if (! cl.value ('D', i) || i < 0)
    {
      cerr << "Must specify whole positive number for malloc death '" << cl.option_value ('D') << "'\n";
      return 1;
    }
  ...
  if (cl.option_present ('d'))
  {
    ...
    if (verbose)
      cerr << "An embedding dummy atom of type '" << dummy_atom_element->symbol() << "' will be added to all groups\n";
  }
  ...
  if (cl.option_present('I'))
  {
    ...
    if (verbose)
      cerr << "Will label the join point with isotope " << isotope_for_join_atom << endl;
  }
  ...
  if (cl.option_present('c'))
  {
    ...
    {
      cerr << "The minimum number of atoms in an rgroup (-c) option must be a whole +ve number\n";
      usage(4);
    }

    if (verbose)
      cerr << "Will discard rgroups with fewer than " << min_atoms_in_rgroup << " atoms\n";
  }

  if (cl.option_present('C'))
  {
    if (! cl.value('C', max_atoms_in_rgroup) || max_atoms_in_rgroup < 1 || max_atoms_in_rgroup < min_atoms_in_rgroup)
    {
      cerr << "The maximum number of atoms in an rgroup (-C) option must be a whole +ve number\n";
      usage(4);
    }
  ...
  if (cl.option_present('h'))
  {
    ...
    {
      cerr << "The remove explicit hydrogens option (-h) option must be combined with the -I option\n";
      usage(3);
    }
...
// For each query, we must assemble the associated query atom numbers
  ...
  {
    int natoms = queries[0]->highest_initial_atom_number();   // or should we use max_atoms_in_query

    for (int i = 1; i < nq; i++)
    {
      if (queries[i]->highest_initial_atom_number() != natoms)
      {
        cerr << "Without the -r option, all queries must have same number of atoms " << queries[i]->highest_initial_atom_number() << " vs " << natoms << endl;
        return i;
      }
    }
