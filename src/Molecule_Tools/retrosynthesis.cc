...
//  private functions
    ...
    void _reportResultDuplicates(const IWString & nameOfReaction, const IWString & nameOfMolecule,
  ...
  public:
    ...
    int process(Molecule & m, Molecule_to_Match & target, IWString_and_File_Descriptor& output);
    int process(Molecule & m, Molecule_to_Match & target, const int radius, IWString_and_File_Descriptor& output);
...
// and smiles.
void
for_debugging_write_molecule(const Molecule& m,
...
//#define DEBUG_RETROSYNTHETIC_Q
  ...
  for (const auto r : radius)
  {
    ...
    for (int reagentIndex = 0 ; reagentIndex < rxn.number_reagents(); ++reagentIndex)
    {
      ...
      int * changed = new_int(matoms + matoms); std::unique_ptr<int[]> free_changed(changed);
...
#ifdef ECHO_CHANGED
    iw_write_array(include_atom_map, highest_atom_map_number, "atoms maps in frag (final):");
#endif 
                          ...
                          IWString_and_File_Descriptor& output)
{
  ...
  for (int i = 0; i < nr; ++i)
  {
    ...
    {
      _reportResultDuplicates(_rxn[radius][i]->name(), m.name(), usmi_here, all_smiles_formed);          
                           ...
                           IWString_and_File_Descriptor& output)
{
  ...
  for (int i = 0; i < number_reactions; ++i)
  {
    ...
    if (_process(m, target, radius, _rxn[radius][i],  newUniqResultSmi, output))
    {
      ...
      _reportResultDuplicates(_rxn[radius][i]->name(), m.name(), newUniqResultSmi, all_smiles_formed);
