...
ECFingerprint::_BondConstant(const Bond& b) const
{
  ...
  cerr << "_BondConstant from " << b.a1() << " to " << b.a2() << " aromatic " << b.is_aromatic()
       << " single " << b.is_single_bond() << '\n';
#endif
  if (b.is_aromatic()) {
    return _bond_magic4;
  }
...
{
#ifdef DEBUG_EC_FINGERPRINT
  cerr << "_AddToRunningSum from atom " << a1 << " type " << shell_info.atom_type(a1) << " to "
       << a2 << " type " << shell_info.atom_type(a2) << '\n';
#endif
  ...
  if (_precise) {
    ...
    //  cerr << "  _precise bond from " << a1 << " to " << a2 << " bond_constant " << bond_constant
    //  << " bit " << b << '\n';
  } else {
...
#ifdef DEBUG_EC_FINGERPRINT
  cerr << "  bond from " << a1 << " to " << a2 << " bond_constant " << bond_constant << " bit " << b
       << " sum " << running_sum << '\n';
#endif
}
...
WriteAllBits::Bit(const ShellInfo& shell_info, const atom_type_t running_sum, const int radius)
{
  ...
  _output << mcopy.smiles() << ' ' << shell_info.name() << " bit " << running_sum << " atom type"
...
void
ECCheckCollisions::Bit(const ShellInfo& shell_info, const atom_type_t running_sum, const int radius)
{
  const atom_number_t a0 = shell_info.a0();
  ...
  if (std::get<1>(tmp) == std::get<1>(f->second) && std::get<2>(tmp) == std::get<2>(f->second)) {
    return;
  }
  ...
  _output << mcopy.smiles() << ' ' << shell_info.name() << " atom type " << shell_info.atom_type(a0)
          << " radius " << _output << "centre atom types " << std::get<1>(tmp) << ' '
          << std::get<1>(f->second) << '\n';
...
void
ECBuildPrecedent::Bit(const ShellInfo& shell_info, const atom_type_t running_sum, const int radius)
{
  ...
  if (f == _bit_count.end())  //  Newly discovered bit.
  {
    ...
    BitCount tmp(mcopy.smiles(), shell_info.atom_type(a0), radius, static_cast<count_type_t>(1));
    ...
    _bit_collisions_avoided++;
    _collision_at_radius[radius]++;
    //  cerr << "Radius bit collision avoided " << running_sum << ' ' << f->second.radius << " vs "
    //  << radius << '\n';
  } else {
    f->second.count++;
  }
                                     ...
                                     IWString& fname) const
{
  ...
  for (const auto& iter : _bit_count) {
    ...
    output << iter.first << sep <<  //  The bit
        iter.second.count << sep << iter.second.smiles << sep << iter.second.center_atom_type << sep
           << iter.second.radius << "\n";
...
void
ECUsePrecedent::Bit(const ShellInfo& shell_info, const atom_type_t running_sum, const int radius)
{
  ...
  for (int r = 0; r <= _max_radius; ++r) {
    ...
    if (std::numeric_limits<count_type_t>::max() == _count[r]) {
      ...
      if (_count[r] > previous_count) {
        ...
        cerr << "ECUsePrecedent::FingerprintingComplete:unexpected count increase, rad " << (r - 1)
             << " count " << previous_count << " radius " << r << " count " << _count[r] << ' '
             << m.name() << '\n';
#endif
      }
...
int
hash_from_file(iwstring_data_source& input, std::unordered_set<atom_type_t>& destination)
{
...
ECBitMeanings::Bit(const ShellInfo& shell_info, const atom_type_t running_sum, const int radius)
{
  ...
  _buffer_current_molecule << mcopy.smiles() << sep << shell_info._m.name() << sep << running_sum
                           << '\n';
}
