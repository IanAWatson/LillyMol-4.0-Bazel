...
MCFirstPass::Build(Command_Line& cl) {
  ...
  else if (cl.value('C', _upper_atom_count_cutoff))
  {
    if (_upper_atom_count_cutoff < _lower_atom_count_cutoff)
    {
      cerr << "Upper atom count cutoff " << _upper_atom_count_cutoff << 
              " must be greater than lower atom count cutoff " << _lower_atom_count_cutoff << endl;
      return 0;
    }
    if (_verbose)
  ...
  if (cl.option_present('R')) {
    if (! cl.value('R', _upper_ring_count_cutoff) ||
      ...
      cerr << "-R option needs a whole number > " << _lower_ring_count_cutoff << endl;
      return 0;
    }
  ...
  if (cl.option_present('Z')) {
    ...
    if (_verbose)
      cerr << "Will discard molecules with ring sizes > " << _upper_ring_size_cutoff << endl;
  }
...
MCFirstPass::Rejected(Molecule& m,
  ...
  (void) NonPrintingCharactersInName(m);
  ...
  if (ExcludeForAtomType(m, counter, rejection_reason)) {
    return 1;
  } else if (_exclude_molecules_with_no_interesting_atoms &&
            ExcludeForNoInterestingAtoms(m, counter, rejection_reason)) {
    return 1;
  } else if (_min_fraction_interesting_atoms > 0.0 &&
                ExcludeForTooFewInterestingAtoms(m, counter, rejection_reason)) {
    return 1;
  } else if (_lower_ring_count_cutoff && 
    ...
    return 1;
  } else if (_upper_ring_count_cutoff &&
...
int
MCFirstPass::DoConvertIsotopes(Molecule & m, MCFirstPassCounter& counter) const {
