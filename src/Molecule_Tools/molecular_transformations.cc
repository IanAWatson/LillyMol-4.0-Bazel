...
/*
  We need a small class to hold any AND or OR relationships within the set of reactions. They will be
  stored in the user specified void ptr of the reaction objects
*/
...
class AndORProb
{
 ...
 //     _active = 1;
    ...
    int andor() const { return _andor;}
    int group() const { return _group;}
    int regioIsomer() const { return _regioIsomer;}

    int active() const { return _active;}
    void set_active (const int s) { _active = s;}

    void set_probability(const double p) { _probability = p;}
...
//  private functions
    ...
    int _read_reaction(const IWString & buffer, Sidechain_Match_Conditions & smc, int & group, const int andor, const double current_group_probability
          , int &regioIsomer);
    int _read_file_of_reactions(const const_IWSubstring & fname, Sidechain_Match_Conditions & smc);
  ...
  public:
    ...
    int build(Command_Line & cl, const char opt, Sidechain_Match_Conditions & smc, const int verbose);
    ...
    void another_match_to_query(const int s) { _hits_to_query[s]++;}
    int  report_matches(std::ostream & output) const;

    int has_starting_molecule() const { return _start_molecule.natoms();}
    ...
    int grouping_is_exhausted(const int reaction_number, const AndORProb * current_andor) const;
    ...
    Molecule & start_molecule() { return _start_molecule;}    // probably should be const

    IWReaction & reaction(const int i) { return *(_reaction[i]);}
    const IWReaction & reaction(const int i) const { return *_reaction[i];}
    resizable_array_p<IWReaction> & reactions() { return _reaction;}
...
{
  const AndORProb * curr_andor = reinterpret_cast<const AndORProb *>(_reaction[reaction_number]->user_specified_void_ptr());    // may be null
  ...
  const AndORProb * prev_andor = reinterpret_cast<const AndORProb *>(_reaction[reaction_number-1]->user_specified_void_ptr());    // may be null
...
Set_of_Reactions::lastRegioIsomerRxnId(const int reaction_number) const
{
  ...
  for (int ndx = reaction_number ; ndx < _number_reactions; ++ndx)
  {
    ...
    const AndORProb * next_andor = reinterpret_cast<const AndORProb *>(_reaction[ndx+1]->user_specified_void_ptr());    // may be null
    if (nullptr == next_andor || next_andor->regioIsomer() != currentRegioIsomer)   // should not happen
      return ndx;
...
Set_of_Reactions::members_in_group(const int reaction_number) const
{
  ...
  {
    const AndORProb * next_andor = reinterpret_cast<const AndORProb *>(_reaction[ndx]->user_specified_void_ptr());    // may be null
...
Set_of_Reactions::set_probability_group(const int reaction_number)
{
  ...
  for (int j = reaction_number; j < gend; ++j)
  {
    ...
    const AndORProb * a = reinterpret_cast<const AndORProb *>(_reaction[j]->user_specified_void_ptr());
...
// need to make sure we have at least one item from each AND group
  ...
  for (int i = 0; i < _number_reactions; ++i)
  {
    const AndORProb * curr_andor = reinterpret_cast<const AndORProb *>(_reaction[i]->user_specified_void_ptr());    // may be null
    if (nullptr == curr_andor)    // should not happen
      continue;
...
int
Set_of_Reactions::append_reagent_names(Molecule & m,
...
{
//cerr << "Set_of_Reactions::set_find_one_embedding_per_start_atom:setting " << s << " for " << _number_reactions << " reactions\n";
  for (int i = 0; i < _number_reactions; i++)
  {
    if (NO_OP_RXN == _reaction[i]->comment())
      continue;
...
//  cerr << "Reaction " << i << " has " << _reaction[i]->number_sidechains() << " sidechains\n";
    for (int j = 0; j < _reaction[i]->number_sidechains(); ++j)
    {
...
Set_of_Reactions::check_for_atom_removals(const int ok_atom_removal_reaction) const
{
  for (int i = 0; i < _number_reactions; i++)
  {
    ...
    cerr << "//t_of_Reactions::check_for_atom_removals:molecular_transformations may not be able to handle reactions that remove atoms, especially with multiple matches\n";

    if (ok_atom_removal_reaction)
      continue;

    cerr << "If you think this will be OK, use the undocumented -H option to give it a try\n";
    cerr << "Failures are likely to be catstrophic in the case of multiple substructure matches\n";
    return 0;
  }
...
static int
final_processing(Molecule & m,
                 int changes_this_molecule,
                 const Set_of_Reactions & sor,
...
static int
in_place_transformation_maybe_append_to_name(IWReaction & rxn,
                                             Molecule & m,
                                        ...
                                        const AndORProb * current_andor) const
{
  ...
  const AndORProb * next_andor = reinterpret_cast<const AndORProb *>(_reaction[reaction_number+1]->user_specified_void_ptr());    // may be null

  if (nullptr == next_andor)   // next reaction is not part of group, exhausted
    return 1;

  if (next_andor->group() != current_andor->group())    // different groups, current group is exhausted
    return 1;

  return current_andor->andor() != next_andor->andor();    // if they are not equal, then grouping is exhausted
}
                                     ...
                                     const AndORProb * current_andor) const
{
  ...
  if (next_andor->group() != current_andor->group())    // different groups, current regioIsomer is exhausted
    return 1;

  if (next_andor->regioIsomer() != current_andor->regioIsomer())    // different regioIsomers, current one is exhausted
    return 1;
...
Set_of_Reactions::shuffle_or_group(const int reaction_number)
{
  ...
  for ( ; ndx < _number_reactions; ++ndx)
  {
    ...
    if (next_andor->andor() != curr_andor->andor())    // not really necesssary as long as groups match
      break;
  }
...
void
Set_of_Reactions::set_active_status(int reaction_number,
                                    const int a)
{
  const AndORProb * ao = reinterpret_cast<AndORProb *>(_reaction[reaction_number]->user_specified_void_ptr());

  const int group = ao->group();

  for ( ;reaction_number < _number_reactions; ++reaction_number)
  {
    AndORProb * ao = reinterpret_cast<AndORProb *>(_reaction[reaction_number]->user_specified_void_ptr());
...
static uint64_t
compute_hash(const resizable_array<int> & do_rxn,
...
//  private functions
    ...
    template <typename O> int  _final_processing(Molecule & m, const int changes_this_molecule, O & output);
    template <typename O> int _process(Molecule & m, int & changes_this_molecule, const int ndx, const resizable_array<int> & do_rxn,
                 resizable_array<int> & actually_made, O & output);

  public:
    Molecular_Transformations_Random_Set_Generator(Set_of_Reactions & s, int * c);

    template <typename T> int process(Molecule & m, int & changes_this_molecule, T & output);
};

Molecular_Transformations_Random_Set_Generator::Molecular_Transformations_Random_Set_Generator(Set_of_Reactions & s, int * c) : _sor(s),
                                                                _number_reactions(s.number_reactions()),
                                                                _changed_by(c)
{
...
int
Molecular_Transformations_Random_Set_Generator::_already_seen(const resizable_array<int> & do_rxn)
{
...
//    do_write(do_rxn, cerr);
  ...
  if (verbose)
    cerr << "Ran " << i << " iterations to produce " << molecules_written << " random molecules. Skipped " << _duplicate_items_skipped_top_level << " duplicates at top level, " << _duplicate_items_skipped_final << " subsequent dups\n";

  return 1;
}

/*
  If we did not use all of the selected reactions, we need to re-check for uniqueness at the end.
  10 20 30 40
  and
  10 21 30 40
...
template <typename O>
int
Molecular_Transformations_Random_Set_Generator::_process(Molecule & m,
                                int & changes_this_molecule,
                                const int ndx,
                                const resizable_array<int> & do_rxn,
                                resizable_array<int> & actually_made,
                                O & output)
{
...
//#define SHOW_FINAL_MOLECULE
    ...
    cerr << "final_processing " << m.name() << ' ' << actually_made.number_elements() << " reactions";
    ...
    if (actually_made.number_elements() < do_rxn.number_elements() && _already_seen(actually_made))   // check partial set
    {
#ifdef SHOW_FINAL_MOLECULE
      cerr << "DISC DUP";
...
template <typename O>
int
Molecular_Transformations_Random_Set_Generator::_final_processing(Molecule & m,
                                                 int changes_this_molecule,
                                                 O & output)
{
  if (elements_to_remove.active())
...
static int
molecular_transformations_random_set(Molecule & m,
                                     int & changes_this_molecule,
                                     int * changed_by,
...
static int
_doAllRegioIsomersAndSitesRecurs(Molecule & m,
                          int & changes_this_molecule,
                          int * changed_by,
                          Set_of_Reactions & sor,
                          int firstReactionNumber,
                          int lastReactionNumber,
                          Substructure_Results &sresults,
                          int firstHit,
                          int lastHit,
                          std::set<IWString> *smilesAlreadyDone,
                          int level)
{
...
// loop over the the hits to process
  ...
  // loop over the  sidechain regioIsomers to be done (each one is a reaction)
    ...
    for (int reactionNumber = firstReactionNumber; reactionNumber <= lastReactionNumber; ++reactionNumber)
    {
      ...
      in_place_transformation_maybe_append_to_name(ri, mcopy, sresults.embedding(thisHit));
      int hitsLeftToDo = hitsToDo - 1;

      Substructure_Results newSresults;
      if (hitsLeftToDo)  // we think we have more to do
      {
        // we must re-do the search because the transformation above will have (potentiallly) changed the atom mappings,
        // and could have removed or added matches to the query.  If it added hits, we bail out - this could mean that
        // the sidechain had a match to the core part of the reaction, and this would/could be an infinate loop
        // if we tried to react the new matches
...
static int
_doAllRegioIsomersAndSites(Molecule & m,
                          int & changes_this_molecule,
                          int * changed_by,
                          Set_of_Reactions & sor,
                          int firstReactionNumber,
...
// Can we process a specific hit number?
  ...
  // now the recursive calls to generate all reactions at the sites specified (one or all) and the regioIsomers (could be just one!)

  if (hit_number >= 0)
  {
    if (! _doAllRegioIsomersAndSites(m, changes_this_molecule, changed_by, sor, reaction_number, last_reaction_number, sresults, hit_number,hit_number))
      return 0;
  }
  else
  {
     if (! _doAllRegioIsomersAndSites(m, changes_this_molecule, changed_by, sor, reaction_number, last_reaction_number, sresults, 0, nhits-1))
       return 0;
  }
...
//  We need the ability to push our state
  ...
  public:
    ...
    Substructure_Results * sresults () { return _sresults;}
    int * changed_by() { return _changed_by;}

    int depth () const { return _depth;}
    int istart () const { return _istart;}
    int jstart () const { return _jstart;}

    void set_depth (int s) { _depth = s;}
    void set_istart (int s) { _istart = s;}
    void set_jstart (int s) { _jstart = s;}

    void change_depth (int s) { _depth += s;}
...
int
MT_OR_Data::identify_matched_atoms(Set_of_Reactions & sor,
                                   Molecule & m)
{
...
/*
  Can we move to the next embedding within the current set of matches, or the next reaction
*/
...
//#define DEBUG_AROMATICITY_PRESEVRATION
  ...
  int * a = new int[_matoms]; std::unique_ptr<int[]> free_a(a);
  ...
  for (int i = 0; i < nr; ++i)
  {
    ...
    if (! ri->all_members_set_in_array(_initial_aromaticity, 1))   // ring was not aromatic to begin with
      continue;
...
static int
molecular_transformations_or_operation(Molecule & m,
                                       MT_OR_Data & mtord,
                                       Set_of_Reactions & sor)
{
  if (1 == mtord.depth())
...
static int
molecular_transformations(Set_of_Reactions & sor,
                          Sidechain_Match_Conditions & smc)
{
...
static int
molecular_transformations(data_source_and_type<Molecule> & input,
                          Set_of_Reactions & sor,
                          Sidechain_Match_Conditions & smc)
{
  Molecule * m;
...
static int
molecular_transformations_tdt(const const_IWSubstring & buffer,
                              Set_of_Reactions & sor)
{
...
static int
molecular_transformations_tdt(iwstring_data_source & input,
                              Set_of_Reactions & sor)
{
  const_IWSubstring buffer;
...
report_results(const Set_of_Reactions & sor, std::ostream & os)
{
  ...
  if (preserve_initial_aromaticity)
  {
    cerr << molecules_with_recovered_aromaticity << " molecules that were able to recover otherwise lost aromaticity\n";
    cerr << molecules_rejected_for_aromaticity_loss << " molecules discarded for loss of aromaticity\n";
  }
...
int
Set_of_Reactions::_read_reaction(const IWString & buffer,
                                 Sidechain_Match_Conditions & smc,
                                 int & group,
                                 const int andor,
                                 const double current_group_probability,
                                 int &regioIsomer)
{
  ...
  std::vector<std::vector<IWString>> smilesLists;  // one vector of smiles for each sidechain
  ...
  //
  // Yes, if there are 300 molecules in the input file, we make 300 (or more) copies of the reaction

  // now get ready for the andor grouping.  If there is already one active, we use it.
  // If it is an AND grouping, each smiles and each mapping of the smiles becomes one more reaction in the AND group,
  // which means each one will generate one output record.
  // if the current grouping is an OR grouping, each mapping of each smiles is part of that OR, and if any one actually matches,
  // the rest of the OR will not be processed
  //
  // If there is no current grouping, we make An AND grouping.  This soes not hurt even if there is only one mapping of one smiles
  // because an AND with only one thing in it is the same as no mapping group.
                         ...
                         Sidechain_Match_Conditions & smc)
{
  ...
  for (int i = 0; i < n; ++i)
  {
    ...
    else if (s.starts_with("PROBABILITY:"))
    {
      ...
      if (! s.numeric_value(current_group_probability) || current_group_probability <= 0.0 || current_group_probability > 1.0)
      {
    ...
    else if (s.starts_with("K:"))
    {
      ...
      if (NO_GROUPING != andor)
        r->set_user_specified_void_ptr(new AndORProb(group, andor, current_group_probability));     // note never freed
      else
        r->set_user_specified_void_ptr(new AndORProb(++group, NO_GROUPING, current_group_probability));      // group number is unique
    ...
    else if (NO_OP_RXN == s)
    {
      ...
      if (NO_GROUPING != andor)
        r->set_user_specified_void_ptr(new AndORProb(group, andor, current_group_probability));
      else
        r->set_user_specified_void_ptr(new AndORProb(++group, NO_GROUPING, current_group_probability));
      _reaction.add(r);
    }
    else if (! _read_reaction(s, smc, group, andor, current_group_probability, regioIsomer))
    {
...
int
Set_of_Reactions::build(Sidechain_Match_Conditions & smc,
                        int & fatal)
{
...
int
Set_of_Reactions::next(Sidechain_Match_Conditions & smc,
                       int & fatal)
{
  fatal = 0;
...
int
Set_of_Reactions::build(Command_Line & cl,
                        const char opt,
                        Sidechain_Match_Conditions & smc,
                        const int verbose)
{
...
static int
display_miscellaneous_options(std::ostream & output,
                              const char flag)
{
  ...
  output << "  -" << flag << " aeuao       Atom Environments match Unmatched Atoms Only\n";
...
{
  Command_Line cl(argc, argv, "vA:E:R:t:i:o:S:m:z:M:ZX:C:jkF:g:bhp:J:Y:uO:lLdU:a:HT:W:P:N:es:r");
  ...
  if (cl.option_present('C'))
  {
    ...
    while (cl.value('C', c, i++))
    {
      ...
      else
      {
        ...
        if (verbose)
          cerr << "Will append '" << append_to_changed_molecules << "' and reaction number to changed molecules\n";
      }
    }
  }
...
//      if (verbose)
//        cerr << "Where just one of several possible sidechain matches used, will append '" << ms << "'\n";
//    }
  ...
  else if (cl.option_present('T'))
  {
    ...
    if (verbose && multiple_reaction_schemes)
      cerr << "Found " << multiple_reaction_schemes << " reaction schemes in control file\n";
  }
  else
  {
    cerr << "Must specify reactions via the -R option, or a control file via the -T option\n";
    usage(1);
  }
...
//        {
//          cerr << "The '-m <number>' option must be followed by a whole positive number\n";
//          usage(91);
//        }
//        enumerate_scaffold_hits_individually = 2;
//      }
...
//  If multiple forms are specified, they must be compatible
    ...
    {
      cerr << "Enumerating multiple scaffold hits (-m each) is incompatible with '-m do=nn'\n";
      usage(51);
    }
...
//    }
  ...
  else
  {
    ...
    {
      if (! molecular_transformations(cl[i], input_type, sor, sidechain_match_conditions))
      {
        rc = i + 1;
        break;
      }
