...
//cerr << "  -m             sort by molecular weight\n";
//cerr << "  -c             sort by number of chiral centres\n";
//cerr << "  -h             sort by number of heteroatoms\n";
//cerr << "  -j             sort by number of aromatic atoms\n";
//cerr << "  -n             sort by number of aromatic rings\n";
//cerr << "  -R             sort by largest ring size\n";
...
class File_Record
{
  ...
  public:
    ...
    int natoms () const { return static_cast<int>(_property[0] + 0.01F);};   // just assumed to be first property
};
...
static int
common_compute_atoms_in_counterions(Molecule & m,
                                     int rc_if_just_one_fragment)
{
...
common_compute_amw_in_counterions(Molecule & m,
                                   atomic_mass_t rc_if_just_one_fragment)
{
  if (m.number_fragments() < 2)
    return rc_if_just_one_fragment;
...
compute_rotatable_bonds(Molecule & m)
{
  ...
  for (int i = 0; i < ne; i++)
  {
    ...
    if (m.ncon(a1) > 1 && m.ncon(a2) > 1 && ! triple_bond_at_either_end(m, b) && ! part_of_otherwise_non_rotabable_entity(m, a1, a2))
      rc++;
  }
...
static int
fetch_sdf_tag_data(const Molecule & m,
                   const IWString & tag,
                   float & v,
                   IW_STL_Hash_Map_int & string_to_ndx)
{
  ...
  for (int i = 0; i < n; i++)
  {
    ...
    cerr << "fetch_sdf_tag_data:invalid value for '" << tag << "' in '" << m.name() << "', value '" << next_record << "'\n";
    return 0;
  }
...
static float
fetch_contents_of_column(const Molecule & m,
                         const int col,
...
compute_atoms_in_largest_ring_system(Molecule & m)
{
  ...
  int * ring_sys = new int[matoms]; std::unique_ptr<int[]> free_ring_sys(ring_sys);

  m.label_atoms_by_ring_system(ring_sys);

  int * atoms_in_ring_system = new_int(nr + 1); std::unique_ptr<int[]> free_atoms_in_ring_system(atoms_in_ring_system);
...
int
File_Record::echo(iwstring_data_source & input,
...
static int
identify_next_group(const resizable_array<File_Record *> & records,
                     int & istart,
                     int & istop)
{
...
//  cerr << "At sorting discontinuity istart " << istart << " istop " << istop << endl;
    ...
    if (istop - istart < min_size_hint)   // not enough to write
      ;
    else if (last_break < 0)    // in the first chunk of molecules, we'll check the next group too
      ;
    else if (istop - istart > (min_size_hint + min_size_hint))   // too much, write previous chunk
    {
      istop = last_break;
      return 1;
    }
    else if (0 == istart)  // otherwise we get problems when just two chunks migh be written
    {
      istop++;
      return 1;
    }
...
static int
echo_set_of_molecules(const resizable_array<File_Record *> & records,
                       int istart,
                       int istop,
...
static int
write_group(const resizable_array<File_Record *> & molecules,
             int istart,
             int istop,
             data_source_and_type<Molecule> & input,
             const IWString & stem,
...
display_dash_m_options(std::ostream & os)
{
  ...
  os << " -M s2f            convert non numeric tags and column contents to arbitrary numeric form - grouped rather than sorted\n";
  ...
  if (cl.option_present('M'))
  {
    ...
    for (int i = 0; cl.value('M', m, i); i++)
    {
      if (m.starts_with("ea="))
      {
        ...
        {
          cerr << "The equal atom count chunk directive (-M ea) only makes sense with the -D option\n";
          usage(2);
        }

        m.remove_leading_chars(3);

        if (! m.numeric_value(splits_to_contain_equal_total_atoms) || splits_to_contain_equal_total_atoms < 2)
        {
        ...
        if (verbose)
          cerr << "Will write " << splits_to_contain_equal_total_atoms << " splits containing roughly equal total atom counts\n";
      }
      ...
      else if ("s2f" == m)
      {
        ...
        if (verbose)
          cerr << "Will convert non numeric tags and column contents to arbitrary numbers\n";
      }
      else if ("help" == m)
    ...
    while (cl.value('k', k, i++))
    {
      ...
      while (k.nextword(token, j, ','))
      {
        ...
        else if (token.starts_with("aicm="))
        {
          ...
          if (! token.numeric_value(presumed_atoms_in_counterion_if_no_counterion) || presumed_atoms_in_counterion_if_no_counterion < 0)
          {
        ...
        else if (token.starts_with("amwcm="))
        {
          ...
          if (! token.numeric_value(presumed_amw_in_counterion_if_no_counterion) || presumed_amw_in_counterion_if_no_counterion < 0.0)
          {
...
//  frp[i] = records + i;
  ...
  if (cl.option_present('D'))
  {
    ...
    if (cl.option_present('e'))
    {
      ...
      if (verbose)
        cerr << "Will try to create chunked output file(s) with at least " << min_size_hint << " molecules in each\n";
    }
