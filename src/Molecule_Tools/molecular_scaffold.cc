...
quick_prop(Molecule & m,int & rotbond,int & hetero)
{
  ...
  for (int i = 0; i < matoms; i++)
  {
    ...
    for(int j = 0; j<ncon; ++j)
    {
       ...
       if (b->nrings())
         continue;
       if(b->is_single_bond() && ncon > 1 && m.atomi(k)->ncon() > 1 && ! triple_bond_at_either_end (m, b) && ! part_of_otherwise_non_rotabable_entity (m, i, k))
         rotbond++;
    }
  }
}
...
static int 
add_isotope_to_neighbors(Molecule & m, atom_number_t i)
{
...
remove_spinach(Molecule & m, int * spinach)
{
  ...
  for(int i=0;i<nr;i++)
  {
    ...
    if(3 == na && 12 ==(check_csp3(m,ring->item(0))*check_csp3(m,ring->item(1))*check_csp3(m,ring->item(2))))
      to_be_removed += (*ring);
    if(4== na && 24 ==(check_csp3(m,ring->item(0))*check_csp3(m,ring->item(1))*check_csp3(m,ring->item(2))*check_csp3(m,ring->item(3))))
      to_be_removed += (*ring);
  }
...
template <typename O>
void 
output_scaffold(Molecule & m, IWString & original,
                int number_ring_systems, O & output)
{
  int matoms = m.natoms ();
  ...
  output << usmi << ' ' << output_rows <<' ' << original << ' ' << matoms << ' ' << rotbond << ' ' <<hetero<<' '<< m.nedges()- matoms + 1 << ' '<< number_ring_systems ;
  if (add_isotope_label)
                   ...
                   O & output)
{
  ...
  const int number_ring_systems = m.label_atoms_by_ring_system_including_spiro_fused(non_linker);
  ...
  int * subset = new int[matoms]; std::unique_ptr<int[]> free_subset(subset);
...
template <typename O>
int
molecular_scaffold(data_source_and_type<Molecule> & input,
                   O & output)
...
molecular_scaffold (int argc, char ** argv)
{
  ...
  *output << "FragmentSMILES ROWID ParentId ParentSMILES ParentNatoms ScaffoldNatoms FragmentNatoms FragmentRotBond FragmentHeteroAtom FragmentNrings FragmentNringSystem";
  if (add_isotope_label)
