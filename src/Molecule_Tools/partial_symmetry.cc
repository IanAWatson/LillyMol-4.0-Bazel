...
#ifdef DEBUG_PARTIAL_SYMMETRY
    cerr << " atom " << i << " is in the current shell\n";
#endif
    const Atom& atom = _m.atom(i);
    for (const Bond * b : atom) {
      const atom_number_t j = b->other(i);
#ifdef DEBUG_PARTIAL_SYMMETRY
      cerr << " is atom " << j << " in radius " << radius << " " << dm[starting_atom * _matoms + j] << '\n';
#endif
...
#ifdef DEBUG_PARTIAL_SYMMETRY
  cerr << "atoms_in_next_shell " << atoms_in_next_shell << '\n';
#endif
...
#ifdef DEBUG_PARTIAL_SYMMETRY
  cerr << "Next shell contains " << _next_shell.size() << " items\n";
#endif
...
#ifdef DEBUG_PARTIAL_SYMMETRY
      cerr << " is atom " << j << " in radius " << radius << " " << dm[starting_atom1 * _matoms + j] << " and " << dm[starting_atom2 * _matoms + j] << '\n';
#endif
...
PartialSymmetry::SymmetricAtRadius() {
  ...
  for (int i = 0; i < _matoms; ++i) {
    ...
    _current_shell << i;
#ifdef DEBUG_PARTIAL_SYMMETRY
    cerr << "Begin expansion from atom " << i << '\n';
#endif
    Expand(i, 1);
  }
...
PartialSymmetry::SortAndAssign(int radius) {
...
#ifdef DEBUG_PARTIAL_SYMMETRY
  cerr << "Actual count next shell " << ndx << '\n';
#endif
  assert(ndx > 0);
  ...
  if (ndx == 2) {
    ...
    atom_number_t a0 = _score[0].atom;
    atom_number_t a1 = _score[1].atom;
