...
static std::unique_ptr<RE2> name_rx;

static std::unique_ptr<RE2> change_name_rx;
            ...
            re2::RE2& change_name_rx)
{
  ...
  {
    cerr << "change_name:change name rx not matched '" << change_name_rx.pattern() << "', molecule name '" << m.name() << "'\n";
    return 0;
  }
  ...
  if (0 == newname.length())
  {
    cerr << "change_name:cannot set name to length zero, rx '" << change_name_rx.pattern() << "', mname '" << m.name() << "'\n";
    return 0;
  }
...
{
  if (!name_matches_name_rx(m.name()))
    return 1;
  ...
  if (cl.option_present('Y'))
  {
    ...
    while (cl.value('Y', y, i++))
    {
      ...
      else if (y.starts_with("chname=") || y.starts_with("CHNAME="))
      {
        ...
        {
          cerr << "The chname regular expression must have capturing goup(s) '" << change_name_rx->pattern() << "' invalid\n";
          return 3;
        }

        if (verbose)
          cerr << "Will change molecule names to part that matches rexexp '" << change_name_rx->pattern() << "'\n";
