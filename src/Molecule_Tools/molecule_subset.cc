...
static int
identify_fragments_with_matched_atoms (Molecule & m, 
...
static int
add_back_atoms_in_small_fragments (Molecule & m,
                                   int * remove_atom,
...
static int
do_write_subset (const Molecule & m,
                 const int * include_bond,
                 const int * hits_in_fragment,
...
static int
handle_no_queries_matching (Molecule & m,
                            Molecule_Output_Object & output)
{
...
static int
molecule_subset_by_bond (Molecule & m,
                         int * hits_in_fragment,
                         Molecule_Output_Object & output)
{
  ...
  int * include_bond = new_int (m.nedges(), 0); std::unique_ptr<int[]> free_include_bond (include_bond);
  ...
  for (int i = 0; i < nq; i++)
  {
    ...
    {
      if (verbose > 1)
        cerr << "Zero hits to query " << i << " '" << queries[i]->comment() << "', only matched " << sresults.max_query_atoms_matched_in_search() << " query atoms\n";
    ...
    {
      cerr << "Requested embedding " << embedding_to_process << " but only " << nhits << " matches\n";
      return 0;
    }
...
static int
do_isotope_at_break_points (Molecule & m,
                            const int * remove_atom)
{
  int nb = m.nedges();
...
static void
identify_atoms_to_be_removed (const Set_of_Atoms & e,
                              int * remove_atom)
{
  if (invert_subset)
...
static int
identify_atoms_proximal_to_retained_atoms (const Molecule & m,
                                           int * remove_atom,
                                           const distance_t include_atoms_within)
{
  const auto matoms = m.natoms();

  for (auto i = 0; i < matoms; ++i)     // ensure that only the number 1 is used in the remove_atom array
  {
    if (remove_atom[i])
      remove_atom[i] = 1;
  }
                 ...
                 Molecule_Output_Object & output)
{
  ...
  int * remove_atom = new int[matoms]; std::unique_ptr<int[]> free_remove_atom (remove_atom);
  ...
  for (int i = 0; i < nq; i++)
  {
    ...
    if (write_each_hit_as_separate_molecule)
    {
      Molecule mcopy (m);
      mcopy.set_name (m.name());
      do_write_each_hit_as_separate_molecule (mcopy, nhits, sresults, remove_atom, hits_in_fragment, output);
      continue;
    }
...
static int
molecule_subset (data_source_and_type<Molecule> & input,
                 Molecule_Output_Object & output)
{
  Molecule * m;
...
static int
molecule_subset (const char * fname,
                 FileType input_type,
                 Molecule_Output_Object & output)
{
...
{
  Command_Line cl (argc, argv, "A:E:S:q:s:o:i:nm:z:vbM:f:T:");
  ...
  if (cl.option_present('T'))
  {
    ...
    if (verbose)
      cerr << "Will include atoms within a spatial distance of " << include_atoms_within << " angstroms\n";
  }
  ...
  if (cl.option_present ('m'))
  {
    ...
    {
      cerr << "Will process a maximum of " << max_matches_to_process << " embeddings per query match\n";
    }
  }
  ...
  if (cl.option_present ('z'))
  {
    ...
    while (cl.value ('z', z, i++))
    {
      ...
      else if (z.starts_with ("nmapp="))
      {
        ...
        write_molecules_not_matching_queries = 1;
        if (verbose)
          cerr << "Will append '" << append_to_unchanged_molecules << "' to the names of non-subsetted molecules\n";
      }
      else
      {
