...
//  We can include atoms that are within a given number of bonds
//  from the matched atoms.
static int include_atoms_within_bonds = 0;

// We can keep track of how many atoms get added by having a
// bond distance.
static extending_resizable_array<int> atoms_added_by_bond_proximity;

// When growing from the matched atoms, it can be convenient
// to include the whole ring when ring atoms are matched.
static int fill_in_partial_rings = 0;
...
usage(int rc)
{
  ...
  cerr << "  -T <dist>      with 3D input, include atoms within <dist> of any matched atom\n";
  cerr << "  -d <nbonds>    include atoms within <nbonds> of matched atoms\n";
...
static int
add_back_atoms_in_small_fragments(Molecule& m, int* remove_atom, const int* hits_in_fragment)
{
  int matoms = m.natoms();
...
static int
write_bond_subsets(Molecule& m, int nhits, const Substructure_Results& sresults, int* include_bond,
...
static int
molecule_subset_by_bond(Molecule& m, int* hits_in_fragment, Molecule_Output_Object& output)
{
  ...
  for (int i = 0; i < nq; i++) {
    ...
    if (0 == nhits) {
      if (verbose > 1) {
        cerr << "Zero hits to query " << i << " '" << queries[i]->comment() << "', only matched "
...
//  For every element in `values` that is non zero, convert to 1.
void
AllNonZeroBecome(int* values, const int n)
{
  for (int i = 0; i < n; ++i) {
    if (values[i] != 0) {
      values[i] = 1;
...
//  For every element of `values` that is `from` change to `to`.
void
Translate(int* values, int n, int from, int to)
{
  for (auto i = 0; i < n; ++i) {
    if (values[i] == from) {
      values[i] = to;
...
static int
IdentifyAtomsInBlllRange(Molecule& m, int* remove_atom, int include_bond)
{
  const int matoms = m.natoms();

  AllNonZeroBecome(remove_atom, matoms);

  int rc = 0;

  for (auto i = 0; i < matoms; ++i) {
    //  start with atoms that were initially NOT being removed
    if (remove_atom[i]) {
      continue;
    }
    ...
    for (auto j = 0; j < matoms; ++j) {
      ...
      if (m.bonds_between(i, j) > include_bond) {
        continue;
      }
  ...
  if (rc) {
    Translate(remove_atom, matoms, 2, 0);
  }
...
do_write_each_hit_as_separate_molecule(Molecule& m,
                                       ...
                                       const Substructure_Results& sresults, int* remove_atom,
                                       int* hits_in_fragment, Molecule_Output_Object& output)
{
  int matoms = m.natoms();
  ...
  for (int i = 0; i < nhits; i++) {
    ...
    if (include_atoms_within_bonds) {
      int tmp = IdentifyAtomsInBlllRange(m, remove_atom, include_atoms_within_bonds);
      if (verbose) {
        ++atoms_added_by_bond_proximity[tmp];
      }
    }
...
// Atoms have been identified in `remove_atom`, some of which
// may include ring atoms. If that is the case, extend what is
// covered in `remove_atom` to all atoms in the ring.
int
FillInPartialRings(Molecule& m, int* remove_atom) {
  ...
  const int matoms = m.natoms();
  // Allocate our own array to make things easy. It could
  // be done with one array, just using different numbers.
  std::unique_ptr<int[]> keep_atom(new_int(matoms));
  int new_atoms_found = 0;
  for (const Ring* r : m.sssr_rings()) {
    // If any members of the ring are NOT removed.
    if (!r->any_members_set_in_array(remove_atom, 0)) {
      continue;
    }
    r->set_vector(keep_atom.get(), 1);
    new_atoms_found = 1;
  }
  ...
  if (new_atoms_found) {
    for (int i = 0; i < matoms; ++i) {
      if (keep_atom[i] && remove_atom[i] == 1) {
        remove_atom[i] = 0;
        ++rc;
      }
    }
...
molecule_subset(Molecule& m, int* hits_in_fragment, Molecule_Output_Object& output)
{
  ...
  for (int i = 0; i < nq; i++) {
    ...
    if (write_each_hit_as_separate_molecule) {
      Molecule mcopy(m);
      mcopy.set_name(m.name());
      do_write_each_hit_as_separate_molecule(mcopy, nhits, sresults, remove_atom, hits_in_fragment,
                                             output);
      continue;
    }
  ...
  if (fill_in_partial_rings) {
    FillInPartialRings(m, remove_atom);
  }
...
{
  Command_Line cl(argc, argv, "A:E:S:q:s:o:i:nm:z:vbd:M:f:T:");
  ...
  if (cl.option_present('M')) {
    ...
    while (cl.value('M', m, i++)) {
      if ("aeuao" == m) {
        ...
        fill_in_partial_rings = 1;
        if (verbose) {
          cerr << "Will add all ring atoms when ring atoms matched\n";
        }
      }
      else if ("help" == m) {
  ...
  if (cl.option_present('d')) {
    if (!cl.value('d', include_atoms_within_bonds) || include_atoms_within_bonds < 1) {
      cerr << "The include bonds within option (-d) must be a whole +ve number\n";
      usage(1);
    }

    if (verbose) {
      cerr << "Will include atoms within " << include_atoms_within_bonds
           << " bonds of matched atoms\n";
    }
  }
  ...
  if (cl.option_present('m')) {
    ...
    if ("each" == m) {
      ...
      cerr << "The number of substructure hits to process must be a whole positive number\n";
      usage(5);
    }
    else if (verbose) {
  ...
  if (verbose) {
    ...
    for (int i = 0; i < atoms_in_molecules_written.number_elements(); i++) {
      if (atoms_in_molecules_written[i]) {
        cerr << atoms_in_molecules_written[i] << " final molecules had " << i << " atoms\n";
      }
    }

    if (include_atoms_within_bonds) {
      cerr << "By including atoms with " << include_atoms_within_bonds << " bonds of matched atoms\n";
      for (int i = 0; i < atoms_added_by_bond_proximity; ++i) {
        if (atoms_added_by_bond_proximity[i])  {
          cerr << i << " atoms added " << atoms_added_by_bond_proximity[i] << " times\n";
        }
      }
    }
  }
