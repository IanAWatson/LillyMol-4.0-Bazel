...
usage(int rc)
{
  ...
  cerr << "  -J <tag>      tag for fingerprints, enter '-J help' for fixed, or descriptor form\n";
...
int
DoAtomPairFingerprint(Molecule & m,
                      ...
                      IWString_and_File_Descriptor & output)
{
  ...
  if (! atom_typing.assign_atom_types(m, atype)) {
    cerr << "AtomPairFingerprint::cannot assign atom types " << m.smiles() << ' ' << m.name() << '\n';
    return 0;
  }
...
DoAtomPairFingerprint(int argc, char ** argv)
{
  ...
  if (cl.option_present('r'))
  {
    ...
    if (verbose)
      cerr << "Will only fingerprint pairs at least " << min_separation << " bonds apart\n";
  }
  ...
  if (cl.option_present('R'))
  {
    ...
    if (verbose)
      cerr << "Will only fingerprint pairs separated by " << max_separation << " bonds or less\n";
  }
