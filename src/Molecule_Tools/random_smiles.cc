...
usage(int rc)
{
  ...
  cerr << " Make randomised smiles from input molecules\n";
...
class Molecule_and_Atom_Type : public Molecule
{
  ...
  public:
    ...
    const int * atype () const { return _atype;}
};
...
class Smiles_and_Index
{
  ...
  public:
    ...
    int place_smiles(const int atype, const int centre, const int width, IWString & output) const;
};

void
Smiles_and_Index::build(Molecule_and_Atom_Type & m,
                        int * tmp,
...
int
Set_of_Random_Smiles::build(Molecule_and_Atom_Type & m,
                            const int ncopy,
                            const std::unordered_set<int> & todo)
{
...
class For_Convolution
{
  private:
    ...
    int _replicates;    // within each Set_of_Random_Smiles how many duplicates should they build
...
//  private functions
    ...
    int _build(Set_of_Random_Smiles & sors, Molecule & m, std::unordered_map<int, int> & atype_count);
...
{
  output << "For_Convolution::debug_print: " << _frame_width << " frame width, " << _replicate_each_type << " replicates each atom type\n";
...
int
For_Convolution::build(Command_Line & cl,
                       char flag)
{
  int nbits = 0;
  ...
  for (int i = 0; cl.value(flag, s, i); ++i)
  {
    ...
    else if (s.starts_with("N="))
    {
      ...
      {
        cerr << "For_Convolution::build:the number of replicates must ba positive '" << s << "' invalid\n";
        return 0;
      }
    }
    else if (s.starts_with("nbits="))
    {
      ...
      {
        cerr << "For_Convolution::build:the number of bits must be a +ve whole number '" << s << "' invalid\n";
        return 0;
      }
    }
    else if (s.starts_with("each="))
    {
      ...
      {
        cerr << "For_Convolution::build:the number of replicates each type must be positive\n";
        return 0;
      }
    }
    else if ("help" == s)
  ...
  for (int i = 0; nullptr != (m = input.next_molecule()); ++i)
  {
    ...
    {
      cerr << "For_Convolution::build:fatal error processing " << m->smiles() << ' ' << m->name() << endl;
      delete m;;
      return 0;
    }
  ...
  std::unordered_map<int, int> atype_count;    // how frequently do we find a given atom type
  ...
  if (verbose)
    cerr << "Found " << atype_count.size() << " different atom types across " << _n << " molecules\n";
  ...
  std::sort(pairs, pairs + natypes, [](const std::pair<int, int> & p1, const std::pair<int, int> & p2) 
                                        {
  ...
  if (nbits > natypes)
  {
    cerr << "For_Convolution::build:only found " << natypes << " different atom types, cannot do " << nbits << " bits, truncating\n";
    nbits = natypes;
  }
...
void
For_Convolution::_display_for_convolution_options(const char flag, std::ostream & output) const
{
  output << " -" << flag << " width=w    frame width for each smiles fragment\n";
  output << " -" << flag << " N=n        number of random smiles replicates (default 10)\n";
                        ...
                        std::unordered_map<int, int> & atype_count)
{
  ...
  {
    cerr << "For_Convolution::_build:cannot assign atom types to " << m.smiles() << ' ' << m.name() << endl;
    return 0;
  }
...
For_Convolution::run(std::ostream & output)
{
  ...
  const int row_size = _replicate_each_type * _bits.number_elements() * (_frame_width + _frame_width + 1);
...
For_Convolution::_generate_row(const int r, IWString & buffer)
{
  ...
  cerr << "Will generate " << _replicate_each_type << " replicates of each of " << _todo.size() << " atom types\n";
...
static int
write_the_permutation(const IWString & mname, 
                       const IWString & rsmi,
...
//  cerr << "rsmi " << rsmi << endl;
    ...
    {
      if (verbose)
        cerr << "After " << iterations << " iterations, only have " << permutations_generated << " permutations '" << m.name() << "'\n";
      break;
    }
  }
...
static int 
make_random_smiles(data_source_and_type<Molecule> & input,
                    std::ostream & output)
{
  Molecule * m;
...
static int
do_convolution(Command_Line & cl, 
               std::ostream & output)
{
  For_Convolution fc;
...
make_random_smiles(int argc, char ** argv)
{
  ...
  if (cl.option_present('n'))
  {
    ...
    {
      cerr << "The number of permutations (-n) option must be followed by a whole positive number\n";
      usage(5);
    }
  ...
  if (cl.option_present('I'))
  {
    ...
    {
      cerr << "The maximum number of iterations (-I) option must be a whole number >= " << number_permutations << endl;
      usage(6);
    }

    if (verbose)
      cerr << "Will try a maximum of " << max_iterations << " iterations for each molecule\n";
  }
  else
  ...
  if (verbose)
  {
    ...
    for (int i = 0; i < permutations_formed.number_elements(); i++)
    {
      if (permutations_formed[i])
        cerr << permutations_formed[i] << " molecules yielded " << i << " random smiles\n";
    }
  }
