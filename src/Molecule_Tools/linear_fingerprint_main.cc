...
int
WriteFixedWidth(Molecule& m,
                const Sparse_Fingerprint_Creator& sfc,
                const int fixed_width,
                IWString_and_File_Descriptor& output)
{
  IW_Bits_Base bits(fixed_width);
  bits.clear();

  const auto& bits_found = sfc.bits_found();

  for (const auto [bit_num, count] : bits_found)
  {
    bits.set(bit_num % fixed_width);
  }
...
LinearFingerprintPipe(iwstring_data_source& input,
  ...
  while (input.next_record(line)) {
    ...
    if (! LinearFingerprintPipe(line, linear_fp_gen, output)) {
      cerr << "LinearFingerprintPipe:invalid input " << line << "' line " << input.lines_read() << endl;
      return 0;
    }
                ...
                IWString_and_File_Descriptor & output)
{
  ...
  if (function_as_tdt_filter) {
    ...
    if (! input.good()) {
      cerr << "LinearFingerprint::cannot open filter " << fname << endl;
      return 0;
    }
...
LinearFingerprint(int argc, char ** argv)
{
  ...
  if (cl.option_present('R'))
  {
    if (! cl.value('R', max_length) || max_length < min_length) {
      cerr << "Max radius (-R) must be larger than min_length " << min_length << endl;
      return 1;
    }
