...
//  cerr << "AddBonds:adding bond between " << zatom << " fg " << functional_group[zatom] << " and " << other << " fg " << functional_group[other] << '\n';
    ...
    if (b->is_aromatic()) {
      query_bond->add_bond_type(SubstructureSearch::BondType::SS_AROMATIC_BOND);
    } else if (b->is_single_bond()) {
      query_bond->add_bond_type(SubstructureSearch::BondType::SS_SINGLE_BOND);
    } else if (b->is_double_bond()) {
      query_bond->add_bond_type(SubstructureSearch::BondType::SS_DOUBLE_BOND);
    } else if (b->is_triple_bond()) {
      query_bond->add_bond_type(SubstructureSearch::BondType::SS_TRIPLE_BOND);
    }
    rc++;
  }
