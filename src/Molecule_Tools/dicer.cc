...
static resizable_array_p<Substructure_Hit_Statistics>
    user_specified_queries_that_fragments_must_contain;
...
class Fragment_Statistics_Report_Support_Specification {
 ...
 public:
  ...
  int fractional_support_level_specified() const { return _fraction >= 0.0F; }

  int set_population_size(int);

  int meets_support_requirement(int c) const { return c >= _int; }
};
...
Fragment_Statistics_Report_Support_Specification::build(const const_IWSubstring& s)
{
  ...
  if (s.numeric_value(_int)) {
    if (_int <= 0) {
      cerr << "Fragment_Statistics_Report_Support_Specification::build:the support level must be a "
              "whole +ve number\n";
      return 0;
    }
  ...
  if (s.numeric_value(_fraction)) {
    if (_fraction < 0.0F || _fraction > 1.0F) {
      cerr << "Fragment_Statistics_Report_Support_Specification::build:the fractional support "
              "level must be a valid fraction\n";
      return 0;
    }
  ...
  cerr << "Fragment_Statistics_Report_Support_Specification::build:the support level specification "
          "must be a fraction or a +ve integer\n";
...
static int
parse_bond_specification_token(const const_IWSubstring& token, int matoms, atom_number_t& a)
{
  int j;
...
{
  os << "Fragments_At_Recursion_Depth:_max_recursion " << _max_recursion << " _max_bonds "
     << _max_bonds << endl;
...
class Dicer_Arguments {
 private:
  ...
  //  Note that this is done once for the parent molecule.
  ...
  int _write_fragment_and_complement(Molecule& m, const IW_Bits_Base& b, int* subset_specification,
 ...
 public:
  ...
  int atoms_in_molecule() const { return _atoms_in_molecule; }

  int set_array_size(int);

  int initialise_parent_molecule_aromaticity(Molecule& m);

  int is_aromatic_in_parent(atom_number_t a) const { return _aromatic_in_parent[a]; }

  int is_aromatic_in_parent(atom_number_t a1, atom_number_t a2, atom_number_t a3,
  ...
  int lower_atom_count_cutoff() const { return _lower_atom_count_cutoff_current_molecule; }

  void set_lower_atom_count_cutoff(int s) { _lower_atom_count_cutoff_current_molecule = s; }

  int upper_atom_count_cutoff() const { return _upper_atom_count_cutoff_current_molecule; }

  void set_upper_atom_count_cutoff(int s) { _upper_atom_count_cutoff_current_molecule = s; }

  void set_write_fragments(int s) { _write_fragments = s; }

  int ok_atom_count(int) const;

  int max_recursion_depth_this_molecule() const { return _max_recursion_depth_this_molecule; }

  //  void set_max_recursion_depth_this_molecule(int s) { _max_recursion_depth_this_molecule = s;}

  void increment_recursion_depth() { _recursion_depth++; }

  void decrement_recursion_depth() { _recursion_depth--; }

  int recursion_depth() const { return _recursion_depth; }

  uint64_t adjust_max_recursion_depth(int bonds_to_break_this_molecule,
                                      uint64_t max_fragments_per_molecule);

  int can_go_deeper() const { return _recursion_depth < _max_recursion_depth_this_molecule; }
  ...
  int* xref_array_this_level() { return _xref + _recursion_depth * _atoms_in_molecule; }

  int update_numbering_for_subset(const int* xref, const Set_of_Atoms& a0, const Set_of_Atoms& a1,
                                  Set_of_Atoms& b0, Set_of_Atoms& b1);
  ...
  //  const resizable_array<int> & fragments_found_this_molecule() const { return
  //  _fragments_found_this_molecule;}

  //  void sort_fragments_found_this_molecule() {
  //  _fragments_found_this_molecule.sort(int_comparitor_larger);}

  int try_more_ring_splitting();

  int write_fragments_found_this_molecule(const Molecule& m,
                                          IWString_and_File_Descriptor& output) const;
  ...
  int fragments_found() const { return _fragments_found_this_molecule.size(); }
...
static void
initialise_xref(const Molecule& m, const Dicer_Arguments& dicer_args, int* xref)
{
...
static int
convert_to_atom_numbers_in_child(const Set_of_Atoms& rbb, const int* xref, Set_of_Atoms& s)
{
...
class Dicer_Transformation {
 ...
 public:
  ...
  int break_bonds(Molecule& m) const { return _break_bonds(m); }
  ...
  int write_bonds_to_be_broken(std::ostream& os) const { return _write_bonds_to_be_broken(os); }

  int debug_print(std::ostream& os) const { return _debug_print(os); }

  int ttype() const { return _ttype; }
};
...
class Bond_Breakage : public Dicer_Transformation {
 ...
 public:
  ...
  Bond_Breakage(atom_number_t z1, atom_number_t z2) : _a1(z1), _a2(z2) { _symmetry_equivalent = 0; }
  ...
  int symmetry_equivalent() const { return _symmetry_equivalent; }

  void set_symmetry_equivalent(int s) { _symmetry_equivalent = s; }
};

int
Bond_Breakage::involves(atom_number_t z1, atom_number_t z2) const
{
  if (_a1 == z1 && _a2 == z2) {
    return 1;
  }
...
class Chain_Bond_Breakage : public Bond_Breakage {
 private:
  ...
  int _process(resizable_array<Molecule*>& components, const IWString& mname,
               Dicer_Arguments& dicer_args, Breakages& breakages, Breakages_Iterator& bi) const;
...
class Fused_Ring_Breakage : public Bond_Breakage {
 ...
 public:
  ...
  Fused_Ring_Breakage(atom_number_t, atom_number_t, atom_number_t, atom_number_t, atom_number_t,
                      atom_number_t);

  ~Fused_Ring_Breakage() { return; }

  atom_number_t a11() const { return _a11; }

  atom_number_t a12() const { return _a12; }

  atom_number_t a21() const { return _a21; }

  atom_number_t a22() const { return _a22; }

  int aromatic_in_parent() const { return _aromatic_in_parent; }

  void set_aromatic_in_parent(int s) { _aromatic_in_parent = s; }
};
...
Fused_Ring_Breakage::Fused_Ring_Breakage(atom_number_t z11, atom_number_t z1, atom_number_t z12,
    ...
    : Bond_Breakage(z1, z2), _a11(z11), _a12(z12), _a21(z21), _a22(z22)
{
...
int
PairTransformation::_debug_print(std::ostream& stream) const
{
  if (_t1) {
...
class Spiro_Ring_Breakage : public Dicer_Transformation {
 ...
 public:
  ...
  atom_number_t aCenter() const { return _aCenter; }
...
Spiro_Ring_Breakage::Spiro_Ring_Breakage(atom_number_t zCenter, atom_number_t z11,
                                         atom_number_t z12, atom_number_t z21, atom_number_t z22)
    : _aCenter(zCenter), _a11(z11), _a12(z12), _a21(z21), _a22(z22)
{
...
class Dearomatise : public Dicer_Transformation {
 ...
 public:
  ...
  void set_ring_atoms(const Set_of_Atoms& r) { _ring = r; }
};
...
class Ring_Bond_Breakage : public Dicer_Transformation, public Set_of_Atoms {
 private:
  ...
  int _do_ring_breaking(Molecule& m, const Set_of_Atoms& rbb, Dicer_Arguments& dicer_args,
 ...
 public:
  ...
  void set_atom_that_must_be_a_ring_atom(atom_number_t a) { _atom_that_must_be_a_ring_atom = a; }
};
...
class Breakages {
 private:
  ...
  int _destroys_too_much_aromaticity(Molecule& m, const int aromatic_atom_count, atom_number_t a1,
                                     atom_number_t a2, atom_number_t a3, atom_number_t a4) const;

 public:

  int number_transformations() const { return _transformations.number_elements(); }

  const Dicer_Transformation* transformation(int i) const { return _transformations[i]; }
...
int
Breakages::_parse_bond_specification_record(const IWString& t, int matoms)
{
  if (2 != t.nwords()) {
...
int
Breakages::get_bonds_to_break_from_text_info(const Molecule& m, int istart)
{
  int rc = 0;
...
int
Breakages::write_bonds_to_be_broken(Molecule& m, Molecule_Output_Object& bbrk_file) const
{
...
class Breakages_Iterator {
 ...
 public:
  ...
  int exhausted() const { return _state >= _n; }
};

Breakages_Iterator::Breakages_Iterator(const Breakages& b) : _n(b.number_transformations())
{
  _state = 0;
...
Breakages_Iterator::Breakages_Iterator(const Breakages_Iterator& rhs) : _n(rhs._n)
{
  _state = rhs._state;
...
static int
initialise_atom_pointers(Molecule& m, int* atom_number)
{
  int matoms = m.natoms();
...
#ifdef NOT_USED_YET
static int
initialise_atom_pointers(Molecule& m, const int* atype, USPVPTR* uspvptr)
{
  const int matoms = m.natoms();
...
Dicer_Arguments::try_more_ring_splitting()
{
  ...
  if (max_fragments_per_molecule > 0 &&
      static_cast<uint64_t>(_ring_splitting_attempted) > max_fragments_per_molecule) {
    return 0;
  }
                                            ...
                                            uint64_t max_fragments_per_molecule)
{
  ...
  auto fragments_that_will_be_produced =
      fard.fragments_at_recursion_depth(max_recursion_depth, bonds_to_break_this_molecule);

  //  #define DEBUG_ADJUST_MAX_RECURSION_DEPTH
#ifdef DEBUG_ADJUST_MAX_RECURSION_DEPTH
  cerr << bonds_to_break_this_molecule << " bonds, max_recursion_depth " << max_recursion_depth
       << " yields " << fragments_that_will_be_produced << " fragments\n";
#endif
  ...
  while (fragments_that_will_be_produced > max_fragments_per_molecule &&
         _max_recursion_depth_this_molecule > 1) {
    _max_recursion_depth_this_molecule--;
    fragments_that_will_be_produced =
        fragments_that_will_be_produced / _max_recursion_depth_this_molecule;

#ifdef DEBUG_ADJUST_MAX_RECURSION_DEPTH
    cerr << "at " << _max_recursion_depth_this_molecule << ", " << bonds_to_break_this_molecule
         << " will produce " << fragments_that_will_be_produced << endl;
#endif
  }
...
Dicer_Arguments::_add_new_smiles_to_global_hashes(const IWString& smiles)
{
  ...
  if (_fragments_found_this_molecule.size() > _next_report_n_fragments) {
    cerr << _current_molecule->name() << " generated " << _fragments_found_this_molecule.size()
         << " fragments\n";
    _next_report_n_fragments += _report_every_n_fragments;
  }
...
static void
add_name_of_parent_to_starting_parent_array(const IWString& smiles, const IWString& name_of_parent,
...
static int
ok_heteromatom_count(const Molecule& m, const int minh)
{
  const int matoms = m.natoms();
...
Dicer_Arguments::contains_fragment(Molecule& m)
{
  ...
  if (fragments_must_contain_heteroatom &&
      !ok_heteromatom_count(m, fragments_must_contain_heteroatom)) {
    return 0;
  }
  ...
  if (f1 == smiles_to_id.end()) {  //  fragment never been seen before
    ...
    //  cerr << "New fragment generated from '" << _current_molecule->name() << "', acc " <<
    //  accumulate_starting_parent_information <<endl;
    if (accumulate_starting_parent_information) {
      add_name_of_parent_to_starting_parent_array(smiles, _current_molecule->name(),
                                                  starting_parent);
    }
...
Dicer_Arguments::is_unique(Molecule& m)
{
  ...
  for (int i = 0; i < n; i++) {
    ...
    if ((*b) == *(_stored_bit_vectors[i])) {
      if (record_presence_and_absence_only)  //  sept 2011, I don't think is useful, make both
                                                     ...
                                                     IWString_and_File_Descriptor& output) const
{
  ...
  //  for (ff_map::const_iterator f = _fragments_found_this_molecule.begin(); f !=
  //  _fragments_found_this_molecule.end(); ++f)
  if (_write_fragments) {
...
int
Dicer_Arguments::produce_fingerprint(Molecule& m, IWString_and_File_Descriptor& output) const
{
  Sparse_Fingerprint_Creator sfp;

  for (ff_map::const_iterator f = _fragments_found_this_molecule.begin();
       f != _fragments_found_this_molecule.end(); ++f) {
...
int
Breakages::_destroys_too_much_aromaticity(Molecule& m, const int aromatic_atom_count,
                                          const Fused_Ring_Breakage& f) const
{
...
static int
identify_attached(const Atom& a, const atom_number_t zatom, const atom_number_t avoid,
...
static bool
two_ring_of_spiro(Molecule& m, const Atom& center, int icenter, atom_number_t atoms[4])
{
  if (4 == center.ncon()) {
    for (int i = 0; i < 4; ++i) {
...
static int
contains_heteroatoms(const Molecule& m, const Ring& r)
{
...
create_fused_ring_breakage(Molecule& m, const atom_number_t a11, const atom_number_t a1,
                           const atom_number_t a12, const atom_number_t a21, const atom_number_t a2,
                           const atom_number_t a22)
{
...
Breakages::identify_spiro_ring_bonds_to_be_broken(Molecule& m)
{
  ...
  for (int i = 0; i < na; i++) {
    ...
    Spiro_Ring_Breakage* s1 =
        new Spiro_Ring_Breakage(i, neighbors[0], neighbors[1], neighbors[2], neighbors[3]);
    Spiro_Ring_Breakage* s2 =
        new Spiro_Ring_Breakage(i, neighbors[2], neighbors[3], neighbors[0], neighbors[1]);
 ...
 */
static int
identify_ring_edge(Molecule& m, const Ring& r, Set_of_Atoms& ring_edge)
{
...
atom_number_t
identify_atom_in_adjacent_ring(Molecule& m, const Set_of_Atoms& s)
{
...
create_ring_bond_breakage(Molecule& m, const Ring* r)
{
  ...
  not_part_of_adjacent_ring.add(
      not_part_of_adjacent_ring[0]);  //  close the ring, we will be processing bonds

  Ring_Bond_Breakage* tmp = new Ring_Bond_Breakage(not_part_of_adjacent_ring, r->is_aromatic());

  atom_number_t atom_in_adjacent_ring =
      identify_atom_in_adjacent_ring(m, not_part_of_adjacent_ring);
...
Breakages::identify_ring_bonds_to_be_broken(Molecule& m)
{
  ...
  for (int i = 0; i < nr; i++) {
    ...
    //  cerr << "Identified ring bond breakages " << not_part_of_adjacent_ring << ' ' <<
    //  m.isotopically_labelled_smiles() << " ex ring " << atom_in_adjacent_ring << endl;
...
usage(int rc)
{
  ...
  cerr << "  -C iso=<n>    write smiles and complementary smiles with isotope <n> on break "
          "points.\n";
  cerr << "  -C atype      write smiles and complementary smiles with atom type labels on break "
          "points\n";
  cerr << "  -a            write only smallest fragments to -C ... output\n";
  cerr << "  -T <e1=e2>    one or more element transformations\n";
  cerr << "  -P ...        atom typing specification (used for isotopes and/or complementary "
          "smiles)\n";
...
class Fragment_Info {
 ...
 public:
  ...
  void set_times_hit(int s) { _times_hit = s; }

  int times_hit() const { return _times_hit; }

  int natoms() const { return _natoms; }

  void set_natoms(int s) { _natoms = s; }

  int initialise_mf_array(Molecule& m);

  int is_subset_of(const Fragment_Info&) const;

  int is_subset_of_another() const { return _is_subset_of_another; }

  void set_is_subset_of_another(int s) { _is_subset_of_another = s; }
};
...
static void
mark_duplicates(Fragment_Info* frag_info, const resizable_array<int>& s)
{
  ...
  for (int i = 0; i < n; i++) {
    ...
    if (isotope_for_join_points || apply_environment_isotopic_labels ||
        isotopic_label_is_recursion_depth) {
...
int
Frag_Info_Comparator::operator()(const Fragment_Info& f1, const Fragment_Info& f2) const
{
...
do_eliminate_fragment_subset_relations()
{
  ...
  for (int i = 0; i < ndx; i++) {
    ...
    cerr << smiles << " ndx = " << fi.ndx() << " times_hit " << fi.times_hit() << " natoms "
         << fi.natoms() << endl;
  }
#endif
...
int
do_apply_atom_type_isotopic_labels(Molecule& m, const atom_number_t j1, const atom_number_t j2,
                                   Dicer_Arguments& dicer_args)
{
  const int* atom_number_in_initial_molecule = dicer_args.atom_number_in_initial_molecule();
  const uint32_t* atype = dicer_args.atom_types();
  atom_number_t a = atom_number_in_initial_molecule[j1];
  m.set_isotope(j1, atype[a]);
...
static int
do_apply_atomic_number_isotopic_labels(Molecule& m, const atom_number_t j1, const atom_number_t j2)
{
...
static int
identify_environment(Molecule& m, const atom_number_t j1)
{
...
static int
do_add_environment_atoms(Molecule& m, const atom_number_t j1, const atom_number_t j2)
{
...
static int
do_apply_environment_isotopic_labels(Molecule& m, const atom_number_t j1, const atom_number_t j2)
{
  const int iso = identify_environment(m, j1);

  if (iso > 0) {
    m.set_isotope(j2, iso);
  }
...
int
Breakages::do_recap(Molecule& m, Dicer_Arguments& dicer_args) const
{
...
Dicer_Arguments::write_fragments_and_complements(Molecule& m,
                                                 IWString_and_File_Descriptor& output) const
{
  int* tmp1 = new int[_nbits];
  std::unique_ptr<int[]> free_tmp1(tmp1);
...
static void
dump_iw_atom_type(Molecule& frag, const Molecule& comp, const int parent_atoms, const int xref[],
                  const uint32_t atypes[], IWString_and_File_Descriptor& output)
{
  //  Molecule mcopy(comp);
  ...
  for (int i = 0; i < parent_atoms; ++i) {
    ...
    //  cerr << " i = " << i << " xref " << xref[i] << " iso " << comp.isotope(xref[i]) << " atype "
    //  << atypes[i] << endl;
...
static void
dump_atom_type(Molecule& frag, const Molecule& comp, const int parent_atoms, const int xref[],
               const uint32_t atypes[], IWString_and_File_Descriptor& output)
{
  ...
  static const char* SYBYL_TYPES[] = {
      "H",  "C3", "C2", "CAR", "C1", "N4", "N3", "N2", "N1", "NAR", "NPL3", "NAM",  "O3",
      "O2", "S3", "S2", "P3",  "BR", "CL", "F",  "I",  "SO", "SO2", "OCO2", "CCAT", "X"};
...
int
Dicer_Arguments::_write_fragment_and_hydrogen(Molecule& m, int* subset_specification,
                                              IWString_and_File_Descriptor& output) const
{
  const int matoms = m.natoms();
  ...
  for (int i = 0; i < matoms; i++) {
    ...
    if (append_atom_count) {
      output << ' ' << s1.natoms() << ' '
             << (static_cast<float>(s1.natoms()) / static_cast<float>(m.natoms()));
    }
...
int
Dicer_Arguments::_write_fragment_and_complement(Molecule& m, const IW_Bits_Base& b,
                                                ...
                                                IWString_and_File_Descriptor& output) const
{
  ...
  //  cerr << "First has " << count_non_zero_occurrences_in_array(subset_specification, matoms) << "
  //  atoms\n";
  ...
  //  apply_isotopes_to_complementary_fragments " << apply_isotopes_to_complementary_fragments <<
  //  endl;
  ...
  if (append_atom_count) {
    output << ' ' << s1.natoms() << ' '
           << (static_cast<float>(s1.natoms()) / static_cast<float>(matoms));
  }
  ...
  if (append_atom_count) {
    output << ' ' << s2.natoms() << ' '
           << (static_cast<float>(s2.natoms()) / static_cast<float>(matoms));
  }
...
int
Dicer_Arguments::_do_apply_isotopes_to_complementary_fragments(Molecule& parent, Molecule& subset,
                                                               const int xref[]) const
{
  ...
  for (int i = 0; i < matoms; i++) {
    ...
    for (int j = 0; j < acon; j++)  //  are any atoms bonded to I not in the subset
    {
      ...
      //    apply_isotopes_to_complementary_fragments << endl;
...
int
Dicer_Arguments::_do_apply_isotopes_to_complementary_fragments(Molecule& parent, Molecule& fragment,
                                                               Molecule& comp) const
{
  const int matoms = parent.natoms();
...
static void
reset_isotope_multiple_connections(Molecule& m, const atom_number_t zatom,
                                   const int* isotope_translation_table)
{
  ...
  //  attached.iwqsort_lambda([] (int i1, int i2) { if (i1 < i2) return -1; if (i1 > i2) return 1;
  //  return 0;});
...
int
Dicer_Arguments::_do_apply_isotopes_to_complementary_fragments_canonical(Molecule& parent, int n,
                                                                         Molecule& frag,
                                                                         Molecule& comp) const
{
  const int matoms = parent.natoms();
  ...
  std::unique_ptr<Atom_Rank_Atom[]> free_ara(ara);
  ...
  std::sort(ara, ara + n, [](const Atom_Rank_Atom& ara1, const Atom_Rank_Atom& ara2) {
    return ara1.canon() < ara2.canon();
  });
#else
  ...
  cerr << "_do_apply_isotopes_to_complementary_fragments_canonical:processing "
       << comp.unique_smiles() << endl;
#endif
  ...
  int* isotope_translation_table = new int[n + 1];
  std::unique_ptr<int[]> free_isotope_translation_table(isotope_translation_table);
...
static int
write_hash_data(IWString_and_File_Descriptor& output, const IW_STL_Hash_Map_uint& smiles_to_bit)
{
  if (verbose) {
    cerr << "Writing " << smiles_to_bit.size() << " smiles->bit relationships\n";
  }

  for (IW_STL_Hash_Map_uint::const_iterator i = smiles_to_bit.begin(); i != smiles_to_bit.end();
       ++i) {
                          ...
                          IWString_and_File_Descriptor& output)
{
  ...
  for (IW_STL_Hash_Map_uint::const_iterator i = molecules_containing.begin();
       i != molecules_containing.end(); ++i) {
  ...
  if (verbose && fsrss.initialised()) {
    float fraction = static_cast<float>(suppressed_by_support_requirement) /
                     static_cast<float>(molecules_containing.size());

    cerr << "Suppressed " << suppressed_by_support_requirement << " of "
         << molecules_containing.size() << " fragments (" << fraction
         << ") because of support requirement\n";
  }
...
static int
read_existing_hash_data_record(const const_IWSubstring& buffer, IW_STL_Hash_Map_uint& smiles_to_bit)
{
  if (buffer.nwords() < 3) {
...
static int
read_existing_hash_data(iwstring_data_source& input, IW_STL_Hash_Map_uint& smiles_to_bit)
{
...
static int
read_existing_hash_data(const IWString& fname, IW_STL_Hash_Map_uint& smiles_to_bit)
{
...
static int
convert_to_atom_numbers_in_current_molecule(const Molecule& m, const atom_number_t a1,
                                            atom_number_t& t1, const atom_number_t a2,
                                            atom_number_t& t2)
{
  ...
  //  cerr << "Gack, no match for atom numbers " << a1 << " (" << t1 << ") and/or " << a2 << " (" <<
  //  t2 << ")\n";
...
atom_number_t
atom_number_in_current_molecule(const Molecule& m, const int* xref, atom_number_t a)
{
  if (xref[a] < 0) {
    return INVALID_ATOM_NUMBER;
  }
...
static int
call_dicer(Molecule& m, Dicer_Arguments& dicer_args, Breakages& breakages, Breakages_Iterator& bi)
{
#ifdef DEBUG_CALL_DICER
  cerr << "Calling dicer, depth " << dicer_args.recursion_depth() << ", molecule is '" << m.smiles()
       << "', can_go_deeper " << dicer_args.can_go_deeper() << endl;
  cerr << "max_recursion_depth " << max_recursion_depth << " max recursion_depth this m "
       << dicer_args.max_recursion_depth_this_molecule() << endl;
#endif
...
int
PairTransformation::_process(Molecule& mol, Dicer_Arguments& arg, Breakages& breakages,
...
int
Chain_Bond_Breakage::_process(Molecule& m0, Dicer_Arguments& dicer_args, Breakages& breakages,
                              Breakages_Iterator& bi) const
{
  ...
  if (stream_for_post_breakage.is_open()) {
    ...
    if (2 != m.create_components(c)) {
      cerr << "Huh, created " << c.number_elements() << " fragments during chain bond breakage "
           << m.smiles() << endl;
      return 0;
    }
...
int
Chain_Bond_Breakage::_process(resizable_array<Molecule*>& components, const IWString& mname,
                              ...
                              Breakages_Iterator& bi) const
{
  ...
  for (int i = 0; i < 2; i++) {
    ...
    if (ci.natoms() > upper_atom_count_cutoff)
      ...
      tmp.add_molecule(&c);
      //    cerr << " c contains " << c.number_fragments() << " fragments, tmp " <<
      //    tmp.number_fragments() << endl;
...
static int
remove_fragments_not_containing(Molecule& m, atom_number_t zatom)
{
...
int
Ring_Bond_Breakage::_do_ring_breaking(Molecule& m, const Set_of_Atoms& rbb,
                                      Dicer_Arguments& dicer_args, Breakages& breakages,
                                      Breakages_Iterator& bi) const
{
  ...
  if (6 == m.atomic_number(r[n - 2]) &&
      6 == m.atomic_number(r[n - 1])) {  //  don't break CC ring bonds, just to keep numbers down
    return 0;
  }
...
#ifdef DEBUG_DO_RING_BREAKING
    cerr << "i = " << i << " Processing atom " << r[i] << " in '"
         << mcopy.isotopically_labelled_smiles() << endl;
#endif
...
static void
identify_extra_ring_attachment(const Molecule& m, const Set_of_Atoms& r, atom_number_t zatom,
                               Set_of_Atoms& exr)
{
  const Atom* a = m.atomi(zatom);
...
static int
atoms_in_two_aromatic_rings(Molecule& m, atom_number_t a1, atom_number_t a2)
{
  int nr = m.nrings();
...
int
Ring_Bond_Breakage::_process(Molecule& m, Dicer_Arguments& dicer_args, Breakages& breakages,
                             Breakages_Iterator& bi) const
{
  ...
  //  cerr << "RBB:Processing atoms " << r << " in " << m.isotopically_labelled_smiles() << " other
  //  ring " << xref[_atom_that_must_be_a_ring_atom] << endl;
  ...
  if (this->_is_arom)  //  I don't think we want to break aromatic bonds, not sure this ever
                       //  happens, besides, the code is wrong, prev does not get updated
  {
    prev = r[0];
...
int
Fused_Ring_Breakage::_process(Molecule& m, Dicer_Arguments& dicer_args, Breakages& breakages,
                              Breakages_Iterator& bi) const
{
  ...
  if (!this->aromatic_in_parent())  //  no need to check anything
    ;
  else if (mcopy.bond_between_atoms(a1, a2)->is_single_bond() && 2 == mcopy.nbonds(a1) &&
           2 == mcopy.nbonds(a2)) {
  ...
  if (dicer_args.is_aromatic_in_parent(a12_in_parent, a1_in_parent, a2_in_parent, a22_in_parent)) {
    if (!mcopy.is_aromatic(a12) || !mcopy.is_aromatic(a1) || !mcopy.is_aromatic(a2) ||
        !mcopy.is_aromatic(a22)) {
      return 0;
    }
  }

  if (1 == mcopy.number_fragments()) {
    cerr << "Fused_Ring_Breakage::molecule has just one fragment "
         << mcopy.isotopically_labelled_smiles() << " " << mcopy.name() << endl;
    cerr << "Removed bonds between atoms " << a11 << " and " << a1 << " AND " << a21 << " and "
         << a2 << endl;
  ...
  if (mcopy.aromatic_atom_count() < initial_aromatic_atom_count - 4) {
    cerr << "Loses too much aromaticity " << mcopy.aromatic_atom_count() << " vs "
         << initial_aromatic_atom_count << endl;
  }
#endif
...
int
Spiro_Ring_Breakage::_process(Molecule& m, Dicer_Arguments& dicer_args, Breakages& breakages,
                              Breakages_Iterator& bi) const
{
  ...
  if (!m.are_bonded(a11, aCenter) || !m.are_bonded(a12, aCenter) || !m.are_bonded(a21, aCenter) ||
      !m.are_bonded(a22, aCenter)) {
    return 0;
  }
...
//  #define DEBUG_DO_RING_SPLITTING
  ...
  if (1 == mcopy.number_fragments()) {
    cerr << "Spiro_Ring_Breakage::molecule has just one fragment "
         << mcopy.isotopically_labelled_smiles() << " " << mcopy.name() << endl;
    cerr << "Removed bonds between atoms " << a11 << " and " << aCenter << " AND " << a12 << " and "
         << aCenter << endl;
    cerr << "Starting molecule " << m.isotopically_labelled_smiles() << endl;
  }
...
int
Dearomatise::_process(Molecule& m, Dicer_Arguments& dicer_args, Breakages& breakages,
                      Breakages_Iterator& bi) const
{
  int* xref = dicer_args.xref_array_this_level();
...
//  #define DEBUG_DICER
#ifdef DEBUG_DICER
  cerr << "Entering dicer " << dicer_args.recursion_depth() << ' ' << m.smiles() << " iterator "
       << bi.state() << endl;
#endif
...
static int
singly_connected_atoms_only(const Molecule& m, atom_number_t zatom, atom_number_t avoid)
{
  const Atom* a = m.atomi(zatom);
...
static int
adjacent_to_terminal_atom(const Molecule& m, atom_number_t zatom)
{
  const Atom* a = m.atomi(zatom);
...
static int
is_terminal_heteroatom(const Molecule& m, atom_number_t zatom)
{
  const Atom* a = m.atomi(zatom);
...
static int
is_amide(const Molecule& m, atom_number_t a1, atom_number_t a2)
{
  const Atom* aa1 = m.atomi(a1);
  const Atom* aa2 = m.atomi(a2);
...
static int
assign_bond_priority(Molecule& m, atom_number_t a1, atom_number_t a2)
{
...
int
Breakages::assign_bond_priorities(Molecule& m, int* priority) const
{
  int n = _transformations.number_elements();
...
void
Breakages::lower_numbers_if_needed(Molecule& m, int& bonds_to_break_this_molecule,
                                   int& max_recursion_depth_this_molecule)
{
  const auto f = fard.fragments_at_recursion_depth(max_recursion_depth_this_molecule,
                                                   bonds_to_break_this_molecule);

  //  cerr << "At recursion depth " << max_recursion_depth_this_molecule << " will create " << f <<
  //  " fragments, cf max " << max_fragments_per_molecule << endl;
  ...
  std::unique_ptr<int[]> free_priority(priority);

  int* to_remove = new_int(n);
  ...
  while (1) {
    ...
    if (fard.fragments_at_recursion_depth(max_recursion_depth_this_molecule,
                                          bonds_to_break_this_molecule - bonds_removed) <=
        max_fragments_per_molecule) {
      break;
    }
  }
  ...
  if (fard.fragments_at_recursion_depth(max_recursion_depth_this_molecule - 1,
                                        bonds_to_break_this_molecule) <=
      max_fragments_per_molecule) {
  ...
  //  Wow, what to do at this stage!! We have removed low priority breakages, lowering the recursion
  //  level will not solve our problem.

  max_recursion_depth_this_molecule--;
  cerr << "Lowered bonds to be broken to " << bonds_to_break_this_molecule
       << " lowering recursion level to " << max_recursion_depth_this_molecule << " frags "
       << fard.fragments_at_recursion_depth(max_recursion_depth_this_molecule,
                                            bonds_to_break_this_molecule)
       << endl;
...
static int
count_side_of_bond(const Molecule& m, atom_number_t astart, int& encountered_ring, int* tmp)
{
  int rc = 1;
...
static int
atoms_on_side_of_bond_satisfy_min_frag_size(const Molecule& m, const Dicer_Arguments& dicer_args,
                                            atom_number_t b1, atom_number_t b2, int* tmp)
{
  if (1 == m.ncon(b1)) {
...
Breakages::identify_bonds_to_break(Molecule& m)
{
  ...
  for (int i = 0; i < nq; i++) {
    ...
    for (int j = 0; j < nhits; j++) {
      ...
      if (e->number_elements() < 2) {
        cerr << "dicer:gack, query " << i << " only has " << e->number_elements()
             << " matched atoms\n";
        return 0;
      }

      if (m.in_same_ring(e->item(0), e->item(1))) {
        cerr << "Cannot process ring bonds, atoms " << e->item(0) << " and " << e->item(1)
             << ", query " << i << " against '" << m.name() << "'\n";
        return 0;
      }
  ...
  if (verbose > 1) {
    cerr << queries_hitting << " of " << nq << " queries produced " << rc << " matches to '"
         << m.name() << "'\n";
  }
...
void
Breakages::_remove_fused_ring_breakage_if_present(atom_number_t a1, atom_number_t a2)
{
  for (int i = _transformations.number_elements() - 1; i >= 0; i--) {
    const Dicer_Transformation* dti = _transformations[i];
...
void
Breakages::_remove_spiro_ring_breakage_if_present(atom_number_t a1, atom_number_t a2)
{
  for (int i = _transformations.number_elements() - 1; i >= 0; i--) {
    const Dicer_Transformation* dti = _transformations[i];
...
void
Breakages::_remove_breakage_if_present(atom_number_t a1, atom_number_t a2)
{
  for (int i = _transformations.number_elements() - 1; i >= 0; i--) {
    const Dicer_Transformation* dti = _transformations[i];
...
static int
write_fully_broken_parent(const Molecule& m, const Breakages& breakages,
...
static void
add_parent_atom_count_to_global_array(const IWString& mname, int matoms,
...
static int
dicer(Molecule& m, IWString_and_File_Descriptor& output)
{
  preprocess(m);
  ...
  if (max_atoms_lost_from_parent > 0) {
    ...
    dicer_args.set_lower_atom_count_cutoff(
        static_cast<int>(static_cast<float>(m.natoms()) * min_atom_fraction));
  }
  else {
    dicer_args.set_lower_atom_count_cutoff(lower_atom_count_cutoff);
  }

  dicer_args.set_upper_atom_count_cutoff(
      std::min(upper_atom_count_cutoff, int(max_atom_fraction * m.natoms())));
  ...
  if (0 == lower_atom_count_cutoff)  //  no need to check
    ...
    bonds_to_break_this_molecule -=
        breakages.discard_breakages_that_result_in_fragments_too_small(m, dicer_args);
  }

  int max_recursion_depth_this_molecule = max_recursion_depth;

  if (max_fragments_per_molecule > 0) {
    breakages.lower_numbers_if_needed(m, bonds_to_break_this_molecule,
                                      max_recursion_depth_this_molecule);
  }
  ...
  if (verbose > 1) {
    cerr << "In '" << m.name() << "', identified " << bonds_to_break_this_molecule
         << " bonds to break\n";
  }
...
static int
dicer(data_source_and_type<Molecule>& input, IWString_and_File_Descriptor& output)
{
  Molecule* m;
...
static int
dicer_tdt_filter(iwstring_data_source& input, IWString_and_File_Descriptor& output)
{
  const_IWSubstring buffer;
...
static int
dicer_tdt_filter(const char* fname, IWString_and_File_Descriptor& output)
{
  iwstring_data_source input(fname);
...
display_misc_B_options(std::ostream& os)
{
  ...
  os << " -B MINFF=<f>      discard fragments that comprise less than <f> fraction of atoms in "
        "parent\n";
  os << " -B MAXFF=<f>      discard fragments that comprise more than <f> fraction of atoms in "
        "parent\n";
  ...
  os << " -B nowritefrags   suppress normal output, useful for complementary frats, or "
        "accumulating fragstat\n";
...
static int
dicer(const char* fname, FileType input_type, IWString_and_File_Descriptor& output)
{
...
display_dash_i_options(std::ostream& os)
{
  ...
  os << "                  1 terminal, 2 aromatic, 3 CG0, 4 !#6G0, 5 Unsat Carbon, 6 Unsat hetero, "
        "7 Amide\n";
  os << " -I enva          specific atoms are added to indicate the environment\n";
  os << "                    Te,         Ar,         Cs,    Hs,      Cu,             Hu,           "
        "  Am\n";
...
dicer(int argc, char** argv)
{
    ...
    for (int j = 0; cl.value('I', i, j); ++j) {
      if ("env" == i) {
        ...
        if (!i.numeric_value(increment_isotope_for_join_points) ||
          ...
          cerr << "The increment isotope values for attachment points directive (-I inc=) must be "
                  "a whole +ve number\n";
          return 2;
        }
  ...
  if (cl.option_present('M')) {
    if (!cl.value('M', upper_atom_count_cutoff) || upper_atom_count_cutoff < 1 ||
  ...
  if (cl.option_present('B')) {
    ...
    while (cl.value('B', b, i++)) {
      if ("term" == b) {
        ...
        if (verbose) {
          cerr << "User specified bond breaking queries will be run in addition to the default "
                  "set\n";
        }
      }
        ...
        if (verbose) {
          cerr << "Fragments must represent a loss of " << max_atoms_lost_from_parent
               << " or fewer atoms from parent\n";
        }
      }
      else if (b.starts_with("MINFF=")) {
        b.remove_leading_chars(6);
        if (!b.numeric_value(min_atom_fraction) || min_atom_fraction < 0.0f ||
            min_atom_fraction > 1.0f) {
        ...
        if (verbose) {
          cerr << "Will only produce fragments that cover at least " << min_atom_fraction
               << " of the atoms in the parent\n";
        }
      }
      else if (b.starts_with("MAXFF=")) {
        b.remove_leading_chars(6);
        if (!b.numeric_value(max_atom_fraction) || max_atom_fraction < 0.0f ||
            max_atom_fraction > 1.0f) {
          cerr << "The max fraction atoms (MAXFF=) directive must specify a fraction\n";
          return 2;
        }

        if (verbose) {
          cerr << "Will only produce fragments that cover at least " << max_atom_fraction
               << " of the atoms in the parent\n";
        }
      }
        ...
        if (0 == b.length()) {
          ...
          if (AtomToTypeSeparator() != b[0] || 1 == b.length()) {
            cerr
                << "To specify the number of heteroatoms, must be of the form 'FMC:heteroatoms:n' '"
                << b << "' invalid\n";
            usage(1);
          }
          ...
          if (!b.numeric_value(fragments_must_contain_heteroatom) ||
              fragments_must_contain_heteroatom < 0) {
            cerr << "The FMC:heteroatom: directive must be followed by a whole +ve number\n";
            usage(1);
          }
        }
        if (verbose) {
          cerr << "Fragments reported must contain at least " << fragments_must_contain_heteroatom
               << " heteroatoms\n";
        }
      }
      else if (b.starts_with("FMC:")) {
        b.remove_leading_chars(4);
        if (!process_cmdline_token(' ', b, user_specified_queries_that_fragments_must_contain,
                                   verbose)) {
        ...
        if (!b.numeric_value(vf_structures_per_page) || vf_structures_per_page < 1) {
          cerr << "The insert parent every n written value must be a whole +ve integer, '" << b
               << "' is invalid\n";
          return 3;
        }

        if (verbose) {
          cerr << "Will insert parent structure into output every " << vf_structures_per_page
               << " fragments written\n";
        }
      }
      else if ("flush" == b) {
        ...
        if (verbose) {
          cerr << "Will report fragment formation every " << report_every_n_fragments
               << " fragments formed\n";
        }
      }
      else if (b == "fsfat") {
      ...
      if (user_specified_queries_that_fragments_must_contain.number_elements()) {
        cerr << "Definied " << user_specified_queries_that_fragments_must_contain.number_elements()
             << " queries for fragments\n";
      }
    }
    ...
    if (bond_break_file.length()) {
      ...
      if (verbose) {
        cerr << "Bonds to be broken written in .sdf form to '" << bond_break_file << "', tag '"
             << bbrk_tag << "'\n";
      }
    }
    else {
  ...
  if (nq) {
    ...
    if (add_user_specified_queries_to_default_rules) {
      cerr << "Have been asked to run queries in addition to default rules, but no extra queries "
              "(-q)\n";
      return 3;
    }
  ...
  if (cl.option_present('K')) {
    ...
    if (read_fname.length()) {
      ...
      if (verbose) {
        cerr << "Read " << smiles_to_id.size() << " smiles to bit number relationships from '"
             << read_fname << "'\n";
      }
    }
  ...
  if (cl.option_present('Z')) {
    ...
    if (stream_for_fully_broken_parents.would_overwrite_input_files(cl, z)) {
      cerr << "Stream for fully broken parent molecules '" << z
           << "' would overwrite input file(s)\n";
      return 1;
    }
  ...
  if (verbose) {
    ...
    for (int i = 0; i < global_counter_bonds_to_be_broken.number_elements(); i++) {
      if (global_counter_bonds_to_be_broken[i]) {
        cerr << global_counter_bonds_to_be_broken[i] << " molecules had " << i
             << " bonds to be broken\n";
      }
    }
  ...
  if (collect_time_statistics) {
    cerr << "Processing took between " << time_acc.minval() << " and " << time_acc.maxval()
         << ", ave " << time_acc.average() << endl;
  }
...
extern "C" {
int
dicer_csharp(int argc, char** argv)
{
