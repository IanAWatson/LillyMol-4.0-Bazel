...
DisplayfOptions(int rc) {
  ...
  cerr << "  -f alarge      determine largest fragment. Keep all fragments with that number of "
          "atoms\n";
  ...
  cerr << "  -f kmfok       compare known salts and parents by molecular formula only - not unique "
          "smiles\n";
...
DisplayDashIOptions(char flag, std::ostream& os) {
  ...
  os << " -" << flag
     << " smarts,<i>    change any atoms matching <smarts> to isotope <i>, e.g. '[2C],0' or "
        "'[#16],4'\n";
...
DisplayDashOOptions(char flag, std::ostream& os) {
  ...
  os << "to enable this use '-" << flag << " none, or -" << flag
     << " ele' to make elements organic\n";
  os << "The other mode is to first check the entire molecule for disallowed elements\n";
  os << "After fragment stripping is done, then check the largest fragment for non-organic "
        "elements\n";
...
int
FileconvConfig::RemoveIsotopes(Molecule& m) {
  int rc = 0;
...
int
FileconvConfig::RemoveLargestFragments(Molecule& m, int remove_largest_fragment) {
  int nf = m.number_fragments();
...
FileconvConfig::ProcessFragments(Molecule& m) {
  ...
  if (1 == remove_largest_fragment)
    return RemoveLargestFragment(m);

  if (remove_largest_fragment > 1)
    return RemoveLargestFragments(m, remove_largest_fragment);
...
FileconvConfig::ApplyAllFiltersInner(Molecule& m,
  ...
  if (remove_isotopic_atoms)
    molecule_changed += RemoveIsotopes(m);
...
int
FileconvConfig::ReportShortestNonBondedDistance(const Molecule& m, std::ostream& output) {
  const int matoms = m.natoms();
...
int
FileconvConfig::OkShortestNonBondedDistance(const Molecule& m) {
  const int matoms = m.natoms();
...
FileconvConfig::ParseRingFiltering(Command_Line& cl) {
  if (cl.option_present('r')) {
    ...
    if (verbose)
      cerr << "Molecules containing fewer than " << lower_ring_count_cutoff << " will be ignored\n";
  }
