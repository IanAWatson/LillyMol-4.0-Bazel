...
// Ran into lots of problems getting things to link, so
// create a reference to something in the problematic
// library to force linking.
static void
UnusedFunction()
{
  IWString notused;
  int notused_either;
  Aprop aprop[2];
  fill_atom_property_array(notused, notused_either, aprop);
}
...
int
Reaction_To_Search::buildRxnPart(const_IWSubstring s, std::vector<Molecule *> & m)
{
  m.clear();
  ...
  for (const const_IWSubstring* smiles : parts)
  {
    std::unique_ptr<Molecule> newMol = std::make_unique<Molecule>();
...
{
  private:
    std::vector<Substructure_Query *> queries;
  ...
  public:
    ...
    ~Reaction_Query_Component()
    {
        for (std::vector<Substructure_Query *>::iterator thisIter = queries.begin() ;  thisIter != queries.end() ; ++thisIter)
                delete *thisIter;
    }
...
int
Reaction_Query::addQuery(const const_IWSubstring & buffer, RxnQueryType typeFlag)
{
  // the buffer might contain a name for the query - if so discard it
...
int
Reaction_Query::addQueryOldFormat(const const_IWSubstring & s)
{
...
int
Reaction_Query::addQueryPartFromString(const_IWSubstring s,
                                       vector<Reaction_Query_Component *> & queryPart,
                                       RxnQueryType typeFlag)
{
  queryPart.clear();
...
int
Reaction_Query::addQueryPartOldFormat(const_IWSubstring &s, vector<Reaction_Query_Component *> &queryPart)
{
  queryPart.clear();
...
int
RXN_Substructure_Search::addQueryFromSmarts(const const_IWSubstring & buffer)
{
  Reaction_Query *newOne = new Reaction_Query();
...
int
RXN_Substructure_Search::addQueriesFromSmilesFile(const const_IWSubstring & fname)
{
...
int
RXN_Substructure_Search::addQueriesFromSmartsFile(const const_IWSubstring & fname)
{
  iwstring_data_source input(fname);
...
int
RXN_Substructure_Search::search()
{
...
int
Reaction_Query::matches(Reaction_To_Search &m)
{
