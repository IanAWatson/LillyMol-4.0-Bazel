...
// Interpret `xzhex` as '0123456789abcdef` and place the resulting
// bits in `destination`. The caller must have allocated an adequate size.
int HexToBinary(const const_IWSubstring& zhex,
                unsigned char * destination);
namespace iwbits {

// Used internally for converting bits data to hex form.
