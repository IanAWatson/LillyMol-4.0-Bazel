                                         ...
                                         Substructure_Results & results)
{
  ...
  for (int i = 0; i < _number_elements; i++)
  {
    ...
    const int tmp = _things[i]->substructure_search(target, results);
...
//  We have enough results for a final answer
    ...
    if (1 == _number_elements)  // The easy case, only one search done.
      return tmp;

    // We have a match, but tmp may be zero. Just return 1.
    return 1;
  }
