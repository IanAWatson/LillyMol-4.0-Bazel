  ...
  for (int i = 0; i < np; i++)
  {
    ...
    else if (53 == zj || 35 == zj || 17 == zj)
    {
      ...
      return 1;
        ...
        16 == zj && 3 == jcon &&
        4 ==
            jbonds)    // Jan 2017 S12=NN(O)NC1=CC(=NC2=C)C PBCHM57260347  - do not aromatise. But what about CC1=N(=O)C=CC=C1
    {
      impossible_aromatic = 1;
      result = NOT_AROMATIC;
      return 1;
    ...
    else if (3 == jcon && 5 == jbonds)    // if both bonds in ring, not aromatic
    {
      ...
      for (int k = 0; k < jcon; k++)
      {
        ...
        if (! p.contains(
                ak))    // found a double bond outside the system, there can't be two in the ring system
          break;
    ...
    if (16 == zj)
    {
      ...
      if (2 == jcon && 2 == jbonds &&
          Pearlman ==
              aromaticity_determination_type)    // dec 2002, Sulphur never aromatic in Pearlman's world
      {
        impossible_aromatic = 1;
        result = NOT_AROMATIC;
        return 1;
...
Molecule::_do_obvious_hcount_adjustments(Kekule_Temporary_Arrays & kta)
{
  ...
  for (int i = 0; i < _number_elements; i++)
  {
    ...
    if (2 == icon)
      ;
    else if (16 == a->atomic_number() && icon > 2 && allow_pipeline_pilot_aromaticity_on_input &&
             0 == a->formal_charge())
    {
      if (4 == icon)
      {
    ...
    if (a->implicit_hydrogens_known())
      ;
        ...
        7 == a->atomic_number() && 3 == a->ncon() && 0 == a->formal_charge() &&
        4 ==
            a->nbonds())    // one of those wierd ND3v4 that hopefully will acquire a positive charge
      ;
                                   ...
                                   int * unshared_pi_electrons)
{
  ...
  int * already_done = new_int(rings_in_molecule, 1);
  std::unique_ptr<int[]> free_already_done(already_done);
  ...
  int * impossible_aromatic_not_used = new_int(rings_in_molecule);
  std::unique_ptr<int[]> free_impossible_aromatic_not_used(impossible_aromatic_not_used);

  int * tmp = new_int(rings_in_molecule, 1);
  std::unique_ptr<int[]> free_tmp(tmp);

  int rc = _determine_aromaticity_of_fused_systems(already_done, pi_e, impossible_aromatic_not_used,
                                                   Simple_4n_plus_2, tmp);
...
Molecule::_bonded_to_heteroatom_outside_system(atom_number_t zatom, const int * aromatic_system)
{
  ...
  for (int i = 0; i < acon; i++)
  {
    ...
    if (is_non_ring_atom(j) || (0 == aromatic_system[j] && ! in_same_ring(zatom, j)))
      return 1;
  }
...
Molecule::find_kekule_form(int * aromatic_atoms, const int * aromatic_bonds)
{
  ...
  if (NULL != aromatic_bonds)
  {
    for (int i = 0; i < nb; i++)
    {
      ...
      _bond_list[i]->set_bond_type(
          single_and_permanent_aromatic);    // does not perturb ring membership
