...
// for backward compatibility, parse the deprecated "isolated_ring" attribute
  ...
  if (! really_gruesome(_aromatic_ring_size, msi, NAME_OF_AROMATIC_RING_SIZE_ATTRIBUTE, _attributes_specified, 3, MAX_NOT_SPECIFIED))
    return 0;

  if (! really_gruesome(_aliphatic_ring_size, msi, NAME_OF_ALIPHATIC_RING_SIZE_ATTRIBUTE, _attributes_specified, 3, MAX_NOT_SPECIFIED))
    return 0;
...
//  os << ind << "(A I atomic_number " << _atomic_number[i] << ")\n";
  ...
  _aromatic_ring_size.write_msi(os, NAME_OF_AROMATIC_RING_SIZE_ATTRIBUTE, indentation);
  _aliphatic_ring_size.write_msi(os, NAME_OF_ALIPHATIC_RING_SIZE_ATTRIBUTE, indentation);
...
Substructure_Atom::_add_component(const msi_object & msi)
{
  ...
  std::unique_ptr<Substructure_Atom_Specifier> tmp =
               std::make_unique<Substructure_Atom_Specifier>();
  ...
  if (_components.empty())    // no operator with first object
    ;
...
Single_Substructure_Query::_parse_ring_specifier_object (const msi_object & msi)
{
  ...
  if (_ring_specification.empty())    // no operators with first object
    ;
...
Single_Substructure_Query::_parse_ring_system_specifier_object(const msi_object & msi)
{
  ...
  if (_ring_system_specification.empty())
    ;
...
//  cerr << "Query atoms created incremented to " << query_atoms_created << endl;
  ...
  if (! _build_no_matched_atoms_between(pst.three_dots())) {
    cerr << "Single_Substructure_Query::_parse_smarts_specifier:invalid 3 dots specification\n";
    return 0;
  }
...
Elements_Needed::construct_from_msi_object (const msi_object & msi)
{
  ...
  int attributes_specified = 0;
  if (! really_gruesome(_hits_needed, msi, NAME_OF_QUERY_HITS_NEEDED_ATTRIBUTE,
                        attributes_specified, 0, MAX_NOT_SPECIFIED))
  {
                     ...
                     const char * attname) const
{
  ...
  _distance.write_compact_representation(os);
...
// Being built from a smarts, and one or more ... or ...{..} construct have been encountered.
// Try to fill in the _nmab array with this info.
int
Single_Substructure_Query::_build_no_matched_atoms_between(const resizable_array<ThreeDots*>& three_dots)
{
  for (ThreeDots * thrdts : three_dots) {
    std::unique_ptr<No_Matched_Atoms_Between> nmab =
                   std::make_unique<No_Matched_Atoms_Between>(thrdts->a1(), thrdts->a2());

    if (! nmab->Initialise(*thrdts)) {
      cerr << "Single_Substructure_Query::_build_no_matched_atoms_between:invalid three dots " << thrdts->qualifier() << endl;
      return 0;
    }

    _nmab.add(nmab.release());
  }
