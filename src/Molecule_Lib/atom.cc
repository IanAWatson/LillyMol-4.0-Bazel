...
template int
Atom::_common_saturation(const std::equal_to<int> &) const;
template int
Atom::_common_saturation(const std::less<int> &) const;
...
int
Atom::other_and_type(atom_number_t my_atom_number, int i, atom_number_t & a, bond_type_t & bt) const
{
  assert(ok());
  assert(ok_index(i));
...
int
Atom::connections(atom_number_t my_atom_number, atom_number_t * others, bond_type_t * bt) const
{
  assert(ok());
  ...
  if (! reasonable_formal_charge_value(f))
  {
    if (_element->organic())
      cerr << "Atom::set_formal_charge: possibly unreasonable formal charge " << f
           << " on atom of type " << atomic_symbol() << endl;
  }
...
Atom::lone_pair_count(int & result)
{
  ...
  if (rc < 0)
  {
    cerr << "Atom::lone_pair_count: Yipes, electron count error, type " << _element->symbol()
         << endl;
...
Atom::pi_electrons(int & pe)
{
  ...
  return _element->pi_electrons(_number_elements +
                                    _implicit_hydrogens,    // total number of connections
                                _formal_charge, pe);
}
...
Atom::valence_ok()
{
  ...
  if (_number_elements + ih > 4)
  {
    if (15 == _element->atomic_number() && 6 == (_number_elements + ih))
      ...
      ;
    else if (16 == _element->atomic_number() && 0 == _formal_charge && 3 == _number_elements &&
             3 == nbonds())
      return 0;
    else
      return 0;
...
int
Atom::next_atom_for_smiles(atom_number_t my_atom_number, int * already_done, int * canonical_order,
                                  ...
                                  int * canonical_order, atom_number_t & next_atom) const
{
  ...
  for (int i = 0; i < _number_elements; i++)
  {
    ...
    cerr << "Choosing next unique atom, " << b << " bond " << bt << " (bcount = " << bcount
         << ", rb = " << rb << ")\n";
#endif
