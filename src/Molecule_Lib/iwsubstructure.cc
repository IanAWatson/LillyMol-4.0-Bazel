...
//cerr << "_determine_if_unmatched_atom_counts_are_present found " << _unmatched_atoms_attached_specified << endl;
...
#ifdef DEBUG_MATCHED_ATOM_POST_CHECK
  cerr << "_unmatched_atoms_attached_specified value " << _unmatched_atoms_attached_specified << endl;
#endif
...
//cerr << "Single_Substructure_Query::_spinach_atoms_match:ais " << ais << endl;
...
//cerr << "Single_Substructure_Query::_spinach_atoms_match:number_inter_ring_atoms " << number_inter_ring_atoms << endl;
                                                ...
                                                Substructure_Results & results)
{
  ...
  if (results.number_embeddings())
    cerr << "     last embedding " << *results.embedding(results.number_embeddings() - 1) << endl;
#endif
...
//cerr << " cmp " << matoms << " and _min_atoms_in_query " << _min_atoms_in_query << endl;
...
Single_Substructure_Query::_unmatched_atoms_attached_matched(const Query_Atoms_Matched& matched_query_atoms,
       ...
       Molecule_to_Match& target) const
{
  for (Substructure_Atom* r : matched_query_atoms)
  {
    if (!r->unmatched_atoms_attached_matches(matched_query_atoms, already_matched, target))
      return 0;
  }
...
#ifdef DEBUG_ATOM_TYPE_GROUPINGS_MATCHED
    cerr << " matched atom " << i << " has type " << atype << " group " << ai->atom_type_group() << endl;
#endif
...
#ifdef DEBUG_ATOM_TYPE_GROUPINGS_MATCHED
      cerr << "   matched atom " << j << " has type " << aj->current_hold_atom()->atom_type() << " group " << aj->atom_type_group() << endl;
#endif
