...
int
determine_atom_type_and_set_tag(const const_IWSubstring & s,
                                IWString & tag,
                                int verbose)
{
  if (0 == tag.length())
...
template <typename T>
int
assign_atom_types_none(Molecule & m,
                       T * invariant)
{
...
template <typename T>
int
assign_atom_types_ch(Molecule & m,
                     T * invariant)
{
  const int matoms = m.natoms();
...
template <typename T>
int
assign_atom_types_z_prime_numbers(Molecule & m,
                                  T * atom_constant)
{
...
template <typename T>
int
assign_atom_types_z(Molecule & m, 
                    T * invariant)
{
  const int matoms = m.natoms();
...
template <typename T>
int
assign_atom_types_z_ring(Molecule & m, 
                         T * invariant)
{
  const int matoms = m.natoms();
...
template <typename T>
int
assign_atom_types_complex(Molecule & m,
                          T * invariant)
{
  const int matoms = m.natoms();
...
template <typename T>
int
assign_atom_types_complex_ring(Molecule & m,
                               T * invariant)
{
...
template <typename T>
int
assign_atom_types_tt_ring (Molecule & m,
                           T * invariant)
{
...
template <typename T>
int
assign_atom_types_sybyl (Molecule & m,
                         T * invariant)
{
...
template <typename T>
int
assign_atom_types_expt (Molecule & m,
                        T * invariant)
{
  const int matoms = m.natoms();
...
template <typename T>
int
assign_atom_types_basic (Molecule & m,
                         T * atype)
{
  const int matoms = m.natoms();
...
template <typename T>
int
assign_atom_types_synthetic_feasibility_expt (Molecule & m, 
                                              T * atype)
{
  const int matoms = m.natoms();
...
static int atomic_number_to_ndx [HIGHEST_ATOMIC_NUMBER + 1] = { 0, 
        1,        // H  1
        2,        // He 2
        2,        // Li 3
        2,        // Be 4
...
static void
determine_ring_status (Molecule & m,
                       int * ring_status)
{
...
// Now look at the remaining aliphatic rings
  ...
  for (int i = 0; i < nr; ++i)
  {
    ...
    for (int j = 0; j < ring_size; ++j)
    {
      ...
      if (3 == ring_status[k])    // has been identified as aromatic, so it is in both aliphatic and aromatic rings
        ring_status[k] = 5;
      else
        ring_status[k] = 4;
    }
                                              ...
                                              T * atype)
{
  ...
  int * ring_status = new_int(matoms); std::unique_ptr<int[]> free_ring_status(ring_status);
...
template <typename T>
int
assign_atom_types_synthetic_feasibility (Molecule & m,
                                         T * atype)
{
  const int matoms = m.natoms();
...
template <typename T>
int
assign_atom_types_cc(Molecule & m,
                     T * atype)
{
  const int matoms = m.natoms();
...
template <typename T>
int
assign_atom_types_nox(const Molecule & m,
                      T * atype)
{
  const int matoms = m.natoms();
...
template <typename T>
int
Atom_Typing_Specification::_assign_atom_types_pharmacaphore (Molecule & m,
                                 T * atype)
{
...
//cerr << "Assigned " << std::count_if(atype, atype + matoms, [] (int t) { return 0 != t;}) << '\n';
  ...
  int * da = new_int(matoms);std::unique_ptr<int[]> free_da(da);
                  ...
                  const int * ncon)
{
  ...
  int * updated_value = new_int(matoms); std::unique_ptr<int[]> free_updated_value(updated_value);
...
template <typename T>
int
Atom_Typing_Specification::assign_atom_types (Molecule & m,
                                 T * atype,
                                 const int * ncon)
{
  std::fill_n(atype, m.natoms(), 0);

  if (IWATTYPE_PPHORE == _type)       // because this owns the charge assigner etc queries. Must be done first
  {
...
template <typename T>
void
Atom_Typing_Specification::_perform_shell_expansion_v1(Molecule & m,
                                                       T * atype) const
{
  ...
  T * tmp = new T[matoms]; std::unique_ptr<T[]> free_tmp(tmp);
...
template <typename T>
void
Atom_Typing_Specification::_perform_shell_expansion_v2(Molecule & m,
                                                       T * atype) const
{
  int need_to_compute_bond_constants = 1;
  ...
  int * complete = new int[matoms + matoms]; std::unique_ptr<int[]> free_complete(complete);   // hopefully one larger allocation faster than two smaller
  ...
  for (auto i = 0; i < _perform_shell_iteration; ++i)
  {
    for (auto j = 0; j < matoms; j++)
    {
      ...
      newtype[j] = _perform_shell_iteration_v2(m, j, atype, complete, need_to_compute_bond_constants, _perform_shell_iteration);
    }
                                ...
                                int radius) const
{
  ...
  int rc = 792 * atype[zatom] + radius - _perform_shell_iteration;   // we want the first iteration of expansion to reflect the proper atom type
  ...
  for (auto i = 0; i < acon; ++i)
  {
    ...
    if (radius > 0)
      rc = rc + _perform_shell_iteration_v2(m, j, atype, complete, need_to_compute_bond_constants, radius - 1);
  }
...
template <typename T>
int
Atom_Typing_Specification::_ust_assign_atom_types_hcount(Molecule & m,
                                                T * atype) const
{
...
template <typename T>
int
Atom_Typing_Specification::_ust_assign_atom_types_pi(Molecule & m,
                                                T * atype) const
{
  int pi;
  for (int i = m.natoms() - 1; i >= 0; i--)
...
/*
  Note that we truncate ring system size considerations at 1. Hard to know what will be most useful...
*/
...
template <typename T>
int
Atom_Typing_Specification::_ust_assign_atom_types_x (Molecule & m,
                                                     T * atype) const
{
...
template <typename T>
int
ust_assign_atom_types_t_ring (Molecule & m,
                              const Ring & r,
                              T * atype)
{
...
template <typename T>
int
Atom_Typing_Specification::_ust_assign_atom_types_arom (Molecule & m,
                                                T * atype) const
{
  if (0 == m.nrings())
    return 1;
                                                ...
                                                T * atype) const
{
  for (int i = m.natoms() - 1; i >= 0; i--)
  {
    ...
    atype[i] += m.ncon(i);     // probably just a bug this still being here. It is not hurting anything...
  }
...
template <typename T>
int
Atom_Typing_Specification::_ust_assign_atom_types_nrings(Molecule & m,
                                                T * atype) const
{
  if (0 == m.nrings())
    return 1;
...
template <typename T>
int
Atom_Typing_Specification::_ust_assign_atom_types_smallest_ring(Molecule & m, T * atype) const
{
  const int nr = m.nrings();
...
template <typename T>
int
Atom_Typing_Specification::_ust_assign_atom_types_largest_ring(Molecule & m, T * atype) const
{
  const int nr = m.nrings();
...
template <typename T>
int
Atom_Typing_Specification::_ust_assign_atom_types_connected_atoms_hash(const Molecule & m, T * atype) const
{
  const int matoms = m.natoms();
...
template <typename T>
int
Atom_Typing_Specification::_ust_assign_atom_types_atomic_symbol_hash_value(const Molecule & m, T * atype) const
{
  const int matoms = m.natoms();
...
template <typename T>
int
Atom_Typing_Specification::_ust_assign_atom_types_formal_charge(const Molecule & m, T * atype) const
{
  const int matoms = m.natoms();
...
template <typename T>
int
Atom_Typing_Specification::_assign_user_specified_type(Molecule & m,
                                 T * atype,
                                 const int * ncon) const
{
  const int matoms = m.natoms();
...
int
Atom_Typing_Specification::_build_pharmacaphore_specification(const const_IWSubstring & s)
{
  IWString myfname(s);
  ...
  if (! input.good())
  {
    cerr << "Atom_Typing_Specification::_build_pharmacaphore_specification:cannot open '" << myfname << "'\n";
    return 0;
  }
...
static int
string_interpolation(const const_IWSubstring & starting_string,
                     IWString & expanded)
{
...
// At this point, it might be a single token ${FOO} or maybe a set of possibilities 
  ...
  if (! interior.contains('|'))    // not a single shell variable, no alternatives avaialble.
    return 0;
...
int
Atom_Typing_Specification::_build_pharmacaphore_specification(const IWString & fname,
...
//  cerr << "Building pp specification '" << buffer << "'\n";
    ...
    if (echo_inputs)
      cerr << "Atom_Typing_Specification::_build_pharmacaphore_specification:reading '" << buffer << "'\n";
...
//  cerr << " expanded               '" << buffer << "'\n";
    ...
    else if (buffer.starts_with("charge_assigner"))
    {
      ...
      {
        cerr << "Atom_Typing_Specification::_build_pharmacaphore_specification, cannot build charge assigner '" << buffer << "'\n";
        return 0;
      }
    }
    else if (buffer.starts_with("donor_acceptor"))
    {
      ...
      {
        cerr << "Atom_Typing_Specification::_build_pharmacaphore_specification, cannot build donor/acceptor assigner '" << buffer << "'\n";
        return 0;
      }
      _donor_acceptor_assigner.set_apply_isotopic_labels(0);   // we allocate our own array, do not want to perturb the incoming molecule
    }
    else if (buffer.starts_with("hydrophobe"))
    {
      ...
      {
        cerr << "Atom_Typing_Specification::_build_pharmacaphore_specification:cannot process pharmacaphore '" << buffer << "'\n";
        return 0;
      }
    }
    else if (buffer.starts_with("write"))
    {
      ...
      {
        cerr << "Atom_Typing_Specification::_build_pharmacaphore_specification:cannot initialise write= stream '" << buffer << "'\n";
        return 0;
      }
    }
    else if ("other" == buffer)    // no directive for what to assign, will get a default type
    ...
    {
      cerr << "Atom_Typing_Specification::_build_hydrophobe_specification:unrecognised directive '" << buffer << "'\n";
      return 0;
    }
  }
...
template int Atom_Typing_Specification::assign_atom_types<int>(Molecule&, int*, int const*);
template int Atom_Typing_Specification::assign_atom_types<unsigned int>(Molecule&, unsigned int*, int const*);
template int Atom_Typing_Specification::assign_atom_types<uint64_t>(Molecule&, uint64_t*, int const*);
