...
//  system so as to identify them
    ...
    ring_sizes_encountered.resize_keep_storage(0);
    ring_sizes_encountered[ri->number_elements()] = 1;
    ...
    if (ri->is_fused())
    {
      for (int j = i + 1; j < nr; j++)
      {
        ...
        {
          ring_sizes_matched++;
          ring_sizes_encountered[rj->number_elements()]++;
        }
        else
          system_rejected = 1;
                                                ...
                                                const int * in_ring_system) const
{
  ...
  for (int i = 0; i < matoms; i++)
  {
    ...
    for (int j = 0; j < acon; j++)
    {
      ...
      if (_distance_to_another_ring.empty())    // we have a min and/or max specification only
      {
