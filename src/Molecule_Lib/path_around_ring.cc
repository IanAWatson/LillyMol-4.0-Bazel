...
int
path_around_edge_of_ring_system(Molecule& m, const int* process_these_atoms, int flag,
                                Set_of_Atoms& s)
{
  int matoms = m.natoms();
  ...
  int* already_visited = new_int(matoms);
  std::unique_ptr<int[]> free_already_visited(already_visited);
...
#ifdef DEBUG_PATH_AROUND_EDGE_OF_RING_SYSTEM
  cerr << "First atom in traversal " << s[0] << '\n';
#endif
...
#ifdef DEBUG_PATH_AROUND_EDGE_OF_RING_SYSTEM
    cerr << "Current atom " << current_atom << ", ncon " << acon << '\n';
#endif
...
#ifdef DEBUG_PATH_AROUND_EDGE_OF_RING_SYSTEM
      cerr << "From " << current_atom << " can we go to to atom " << b->other(current_atom)
           << " bond in " << b->nrings() << " rings , flag? "
           << (flag == process_these_atoms[b->other(current_atom)]) << '\n';
#endif
...
#ifdef DEBUG_PATH_AROUND_EDGE_OF_RING_SYSTEM
      cerr << "Check first atom " << first_atom << " vs " << j << " have " << s.number_elements()
           << " items\n";
#endif
...
#ifdef DEBUG_PATH_AROUND_EDGE_OF_RING_SYSTEM
    cerr << "path_around_edge_of_ring_system '" << m.name() << " continuing from " << current_atom
         << " to " << next_atom
         << '\n';  //" '" << m.smarts_equivalent_for_atom (current_atom) << "'\n";
#endif
