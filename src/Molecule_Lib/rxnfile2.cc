...
template <typename T>
int
write_rxn_smiles(const Reaction_Smiles_Options &, T & output);
...
template <typename T>
void
write_set_of_reactions(ISIS_RXN_FILE_Molecule * m, const int n, const bool plus_rather_than_dot,
                       T & output)
{
  for (int i = 0; i < n; ++i)
  {
...
template int
RXN_File::write_rxn_smiles(const Reaction_Smiles_Options &, IWString_and_File_Descriptor &);
template int
RXN_File::write_rxn_smiles(const Reaction_Smiles_Options &, std::ostream &);
                                     ...
                                     int component_grouping_is_plus)
{
  ...
  else if (token.starts_with('|') && token.ends_with('|'))
  {
    ...
    {
      cerr
          << "RXN_File::build_from_reaction_smiles:plus present as component grouping, and Chemaxon extension, impossible\n";
      return 0;
    }
...
#ifdef DEBUG_BUILD_FROM_REACTION_SMILES
  cerr << "COmment set to '" << _comment << "', chemaxon ? " << chemaxon_fragment_data.active()
       << endl;
#endif
...
//  cerr << "ChemAxonFragmentData::Build token '" << token << "'\n";
    ...
    else if (previous_was_f && isdigit(token[0]))
    {    // && token.contains('.') && isdigit(token.last_item())) {
      if (! _add_group(token))
      {
...
{
  output << "ChemAxonFragmentData:with " << _connected_group.size() << " connected groups\n";
...
int
ReactionStringRep::Build(const const_IWSubstring & rxn_smiles, const char component_separator)
{
...
RXN_File::at_least_some_mapped_atoms_common_btw_reagents_and_products()
{
  ...
  cerr
      << "RXN_File::at_least_some_mapped_atoms_common_btw_reagents_and_products returning false, nr "
      << _nr << endl;
  return 0;
}
...
static int
is_duplicate(const atom_number_t a1, const atom_number_t a2, const Set_of_Atoms & already_found)
{
...
int
RXN_File::_identify_atoms_with_different_bonding(const ISIS_RXN_FILE_Molecule & m, const Ring & r,
...
static void
grow_aromatic_system(Molecule & m, const int rng, int * fused_aromatic_system_id, const int id)
{
...
RXN_File::_fix_kekule_differences_reagent(const int rgnt)
{
  ...
  for (int i = 0; i < nr; ++i)
  {
    cerr << "fused_aromatic_system " << fused_aromatic_system[i] << " ring " << *r.ringi(i)
         << " ring_already_done " << ring_already_done[i] << endl;
  }
#endif
  ...
  for (int i = 0; i < nr; ++i)
  {
    ...
    for (int j = i; j < nr; ++j)    // note we start with ring I
    {
...
#ifdef DEBUG_FIX_KEKULE_DIFFERENCES
      cerr << "System with " << i << " and " << j << " ring_already_done " << ring_already_done[i]
           << endl;
#endif
      ...
      if (_identify_atoms_with_different_bonding(r, *rj, a1, a2, std::equal_to<int>(),
        ...
        diff = 1;
      else if (_identify_atoms_with_different_bonding(r, *rj, a1, a2, std::not_equal_to<int>(),
                                                      s))    // bonding the same
        diff = 0;
      else
        continue;
...
int
RXN_File::_bond_the_same_in_product(const ISIS_RXN_FILE_Molecule & r, const Bond & b) const
{
  ...
  for (int i = 0; i < ring_size; ++i)
  {
    ...
    if (_same_connectivity_and_bonds_in_product(r, j))    // no change
      ;
                 ...
                 rgnt, r.atom_map()[j]))    // gain or loss of heavy atom, but Hydrogen atom present
      ;
    else
      return 1;
...
RXN_File::_loss_or_gain_of_singly_bonded_atom(const int rgnt, const int am) const
{
  ...
  if (acon1 > acon2)    // losing a connection, product must have a Hydrogen atm
  {
    ...
    if (7 == a1->atomic_number() && 1 == a1->formal_charge() && 3 == acon1 && 2 == acon2 &&
        0 == a2->formal_charge())
      return 1;
  ...
  else if (acon1 < acon2)    // gaining a connection, reagent must have a Hydrogen atom
  {
    ...
    if (7 == a1->atomic_number() && 1 == a2->formal_charge() && 3 == acon2 && 2 == acon1 &&
        0 == a1->formal_charge())
      return 1;
...
static int
_contains_duplicate_atom_map_numbers(const int n, const ISIS_RXN_FILE_Molecule * m, int * mapped)
{
  int rc = 0;
...
/*
  ...
  Maybe I need to write a different function to handle those cases....
*/
...
RXN_File::remove_unchanging_fragments()
{
  ...
  for (int i = 0; i < _nr; ++i)
  {
    ...
    cerr << "RXN_File::remove_unchanging_fragments:reagent " << i << " had " << r.number_fragments()
         << " fragments\n";
    cerr << r.smiles() << endl;
#endif
...
RXN_File::remove_unchanging_components()
{
  ...
  for (int i = 0; i < _nr; ++i)
  {
    ...
    for (int j = 0; j < _np; ++j)
    {
      ...
      for (int k = i; k < (_nr - 1); ++k)    // reagent I and product j are the same.  Remove them
      {
        _reagent[k] = std::move(_reagent[k + 1]);
      }
      for (int k = j; k < (_np - 1); ++k)    // reagent I and product j are the same.  Remove them
      {
...
#ifdef DEBUG_REMOVE_UNCHANGING_FRAGMENTS
  cerr << "RXN_File::_remove_common_fragments:remving common frags btw " << r.smiles() << " and "
       << p.smiles() << endl;
#endif
...
#ifdef DEBUG_REMOVE_UNCHANGING_FRAGMENTS
  cerr << "_same_atoms_and_bonds:reagent fragment " << rf << " product fragment " << pf << endl;
  cerr << "atoms_in_fragment " << r.atoms_in_fragment(rf) << " and " << p.atoms_in_fragment(pf)
       << endl;
#endif
...
#ifdef DEBUG_REMOVE_UNCHANGING_FRAGMENTS
    cerr << " atom " << i << " in reagent fragment " << r.smarts_equivalent_for_atom(i) << " map "
         << amap << endl;
#endif
...
#ifdef DEBUG_REMOVE_UNCHANGING_FRAGMENTS
    cerr << " atom " << x << " in product fragment " << p.smarts_equivalent_for_atom(x) << endl;
#endif
...
#ifdef DEBUG_REMOVE_UNCHANGING_FRAGMENTS
  cerr << "_atoms_the_same comparing atom " << a1 << " " << m1.smarts_equivalent_for_atom(a1)
       << " with " << a2 << " " << m2.smarts_equivalent_for_atom(a2) << endl;
#endif
