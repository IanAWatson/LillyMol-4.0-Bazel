...
//  In reactions, we want a substitution to be able to preserve stereochemistry
    ...
    formal_charge_t net_formal_charge() const;  // same as net_formal_charge();
    ...
    const Atom& operator[](int atom) const {
      return *_things[atom];
    }
...
extern int create_file_with_appropriate_name(const const_IWSubstring &, IWString &, FileType, int = 0);
