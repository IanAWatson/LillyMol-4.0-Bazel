...
T * ReadProtoQueryFile(const const_IWSubstring fname)
{
  ...
  if (! input.good())
  {
    cerr << "ReadProtoQueryFile:cannot open '" << fname;
    return nullptr;
  }
  ...
  {
    cerr << "ReadProtoQueryFile:cannot parse proto\n";
  ...
  {
    cerr << "ReadProtoQueryFile:cannot build query from proto\n";
...
ReadFileOfProtoQueries(const const_IWSubstring fname, resizable_array_p<T>& queries)
{
  ...
  if (! input.good())
  {
    cerr << "ReadFileOfProtoQueries:cannot open '" << fname << "'\n";
    return 0;
  }
  ...
  while (input.next_record(line))
  {
    ...
    if (nullptr == q)
    {
      cerr << "ReadFileOfProtoQueries:cannot process '" << line << "'\n";
      return 0;
    }
...
/*else if (mytoken.starts_with("ISIS:"))
  ...
  else if (mytoken.starts_with("PROTO:"))
  {
    ...
    if (nullptr == q)
    {
      cerr << "process_queries::cannot read proto file '" << mytoken << "'\n";
      return 0;
    }
    queries.add(q);
  }
  else if (mytoken.starts_with("PROTOFILE:"))
  {
    ...
    {
      cerr << "process_queries:cannot read file of proto files '" << mytoken << "'\n";
      return 0;
    }
  }
  else
