...
GFP_L::construct_from_tdt (IW_TDT & tdt, int & fatal)
{
  ...
  else if (threshold_column >= 0)
  {
    ...
    {
      cerr << "GFP_L::construct_from_tdt: no " << threshold_column << " column in '" << _id << "'";
      if (::threshold > 0.0)
  ...
  if (max_cluster_size_tag.length() > 0)
  {
    ...
    {
      cerr << "GFP_L::construct_from_tdt: missing or invalid '" << max_cluster_size_tag << "' in tdt\n";
      return 0;
    }
...
static int
do_leader_on_clusters (Cluster & cluster,
...
template void resizable_array_base<GFP_L*>::iwqsort<Distance_Comparitor>(Distance_Comparitor&);
template void iwqsort<GFP_L*, Distance_Comparitor>(GFP_L**, int, Distance_Comparitor&);
template void iwqsort<GFP_L*, Distance_Comparitor>(GFP_L**, int, Distance_Comparitor&, void*);
template void compare_two_items<GFP_L*, Distance_Comparitor>(GFP_L**, Distance_Comparitor&, void*);
template void move_in_from_left<GFP_L*, Distance_Comparitor>(GFP_L**, int&, int&, int, Distance_Comparitor&, void*);
//template void move_in_from_right<GFP_L, Distance_Comparitor>(GFP_L**, int&, int&, Distance_Comparitor&);
...
static int
form_cluster_threshold (int icentre,
                        Cluster & cluster,
              ...
              int max_cluster_size_this_molecule)
{
  ...
  if (max_cluster_size_this_molecule)
    return form_cluster_max_cluster_size(icentre, cluster, my_threshold, max_cluster_size_this_molecule);
  else
        ...
        IWString_and_File_Descriptor & output)
{
  ...
  while (items_selected < pool_size)
  {
    ...
    if (max_cluster_size_tag.length() && pool[icentre].max_cluster_size(max_cluster_size_this_molecule))
      ;
    else if (max_cluster_size_column >= 0 && pool[icentre].max_cluster_size(max_cluster_size_this_molecule))
      ;
    else
    ...
    if (verbose > 1)
    {
      cerr << "Start cluster " << clusters_found << ". ndx " << icentre << ", threshold = " << my_threshold;
...
static int
do_previously_selected_file (IW_TDT & tdt,
                             similarity_type_t t)
{
...
{
  Command_Line cl (argc, argv, "vs:I:E:t:F:P:W:X:rH:S:C:Y:V:R:m:M:A:L:a:D:");
  ...
  if (cl.option_present('D'))
  {
    ...
    while (cl.value('D', d, i++))
    {
      ...
      else if ("sbcr" == directive)
      {
        ...
        if (verbose)
          cerr << "Variable sub-cluster threshold ratio " << sub_cluster_threshold_ratio << endl;
...
// that the pool is built first
  ...
  if (cl.option_present('A'))
  {
    ...
    if (cl.option_present('L'))
    {
      ...
      stream_for_discarded_by_previously_selected.open(fname.null_terminated_chars(), std::ios::out);
      if (! stream_for_discarded_by_previously_selected.good())
      {
        cerr << "Cannot open stream for molecules discarded by previously selected '" << fname << "'\n";
        return 4;
      }
