...
static float * _fingerprint_weight = nullptr;
static float * _sparse_fingerprint_weight = nullptr;
static float * _fixed_size_counted_fingerprint_weight = nullptr;

static int * _bits_in_fingerprint = nullptr;
...
static float * _sparse_fingerprint_scaling_factor = nullptr;
...
static int * _sparse_fingerprint_distance_metric = nullptr;

static int multiconformer_fingerprints_present = 0;

static float * _multiconformer_01_weight = nullptr;
static int * _multiconformer_01_distance_metric = nullptr;
static float * _multiconformer_fixed_size_counted_weight = nullptr;
static int * _multiconformer_fixed_size_counted_distance_metric = nullptr;
static float * _multiconformer_sparse_weight = nullptr;
static int * _multiconformer_sparse_distance_metric = nullptr;
...
static int * _fixed_size_counted_fingerprint_distance_metric = nullptr;

//  The tag for each fingerprint
//  Note that these are never freed

static IWString * _fingerprint_tag = nullptr;
static IWString * _sparse_fingerprint_tag = nullptr;
static IWString * _fixed_size_counted_fingerprint_tag = nullptr;
static IWString * _multiconformer_01_fingerprint_tag = nullptr;
static IWString * _multiconformer_fixed_size_counted_fingerprint_tag = nullptr;
static IWString * _multiconformer_sparse_fingerprint_tag = nullptr;
...
static int * _fingerprint_type = nullptr;
                   ...
                   int mcsparse) // multiconformer, sparse
{
  ...
  if (cfp_to_allocate > 0)
  {
    ...
    if (nullptr == _fixed_size_counted_fingerprint_tag)
    {
  ...
  if (sfp > 0)
  {
    ...
    if (nullptr == _sparse_fingerprint_tag)
    {
  ...
  if (mc01 > 0)
  {
    ...
    if (nullptr == _multiconformer_01_fingerprint_tag)
    {
  ...
  if (mcfsc > 0)
  {
    ...
    if (nullptr == _multiconformer_fixed_size_counted_fingerprint_tag)
    {
  ...
  if (mcsparse > 0)
  {
    ...
    if (nullptr == _multiconformer_sparse_fingerprint_tag)
    {
...
static std::unique_ptr<RE2> fingerprint_rx; // ("^FP[A-Z]*<.+[0-9]>");
...
IW_General_Fingerprint::operator = (const IW_General_Fingerprint & rhs)
{
  ...
  if (number_fingerprints_to_use_in_computations > 0 && nullptr == _fingerprint)
    need_to_allocate = 1;
  else if (_number_sparse_fingerprints > 0 && nullptr == _sparse_fingerprint)
    need_to_allocate = 1;
...
IW_General_Fingerprint::_allocate_fingerprint_array()
{
  ...
  if (_number_sparse_fingerprints > 0)
  {
    ...
    assert (nullptr != _sparse_fingerprint);
  }
  ...
  if (number_fixed_size_counted_fingerprints_to_allocate > 0)
  {
    ...
    assert (nullptr != _fixed_size_counted_fingerprint);
  }
  ...
  if (number_multiconformer_01_fingerprints_in_file > 0)
  {
    ...
    assert (nullptr != _multiconformer_01);
  }
  ...
  if (number_multiconformer_fixed_size_counted_fingerprints_in_file > 0)
  {
    ...
    assert (nullptr != _multiconformer_fixed_size_counted);
  }
  ...
  if (number_multiconformer_sparse_fingerprints_in_file > 0)
  {
    ...
    assert (nullptr != _multiconformer_sparse);
  }
...
IW_General_Fingerprint::_default_values()
{
  ...
  _sparse_fingerprint = nullptr;

  _fixed_size_counted_fingerprint = nullptr;

  _multiconformer_01 = nullptr;

  _multiconformer_fixed_size_counted = nullptr;

  _multiconformer_sparse = nullptr;
...
/*
  Notice that we don't initialise the arrays to nullptr in the constructor
...
IW_General_Fingerprint::~IW_General_Fingerprint()
{
  ...
  if (nullptr != _fixed_size_counted_fingerprint)
    delete [] _fixed_size_counted_fingerprint;


  if (nullptr !=  _multiconformer_01)
    delete [] _multiconformer_01;

  if (nullptr != _multiconformer_fixed_size_counted)
    delete [] _multiconformer_fixed_size_counted;

  if (nullptr != _multiconformer_sparse)
...
IW_General_Fingerprint::initialise (const IW_TDT & tdt)
{
  ...
  if (! fingerprint_rx)
    set_fingerprint_regular_expression(fingerprint_rx_default);
  ...
  {
    cerr << "IW_General_Fingerprint::_initalize_auto: no fingerprints '" << fingerprint_rx->pattern() << "' in tdt\n";
...
//#define ECHO_FINGERPRINT_DATA
  ...
  if (nullptr == _fingerprint && nullptr == _sparse_fingerprint && nullptr == _fixed_size_counted_fingerprint)
    _allocate_fingerprint_array();
...
initialise_fingerprints (Command_Line & cl, int verbose)
{
  ...
  {
    set_fingerprint_regular_expression(fingerprint_rx_default);
      cerr << "All fingerprints matching '" << fingerprint_rx->pattern() << "' will be processed\n";
  } 
  else
  {
...
{
  if (nullptr != _fingerprint_weight)
    delete [] _fingerprint_weight;
  if (nullptr != _sparse_fingerprint_weight)
    delete [] _sparse_fingerprint_weight;
  if (nullptr != _fixed_size_counted_fingerprint_weight)
    delete [] _fixed_size_counted_fingerprint_weight;
  if (nullptr != _bits_in_fingerprint)
    delete [] _bits_in_fingerprint;

  if (nullptr != _fingerprint_type)
    delete [] _fingerprint_type;
  if (nullptr != _sparse_fingerprint_type)
  ...
  if (nullptr != _sparse_fingerprint_scaling_factor)
    delete [] _sparse_fingerprint_scaling_factor;
  if (nullptr != _sparse_fingerprint_distance_metric)
    delete [] _sparse_fingerprint_distance_metric;

  if (nullptr != _dense_fingerprint_distance_metric)
