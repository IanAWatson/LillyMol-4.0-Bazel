                         ...
                         const IWString & marker_column_name)
{
  ...
  {
    cerr << "Invalid experimental column " << experimental_column << " only " << columns_in_input << " columns in input\n";
    return 0;
  }
  ...
  for (int i = 0; i < predicted_column.number_elements(); i++)
  {
    ...
    {
      cerr << "Invalid predicted value column " << p << " only " << columns_in_input << " columns in input\n";
      return 0;
    }
  }
...
//  when doing randomised breaking of ties, we need a random number with each prediction
  ...
  public:
    ...
    experimental_value_t obs() const { return _experimental;}
    experimental_value_t pred (int c) const { return _predicted[c];}
    ...
    int parse_buffer (const const_IWSubstring & buffer,
              int experimental_column,
              const resizable_array<int> & predicted_column,
              int marker_column,
              int & fatal);
...
int
Predicted_Values::parse_buffer (const const_IWSubstring & buffer,
                                int experimental_column,
                                const resizable_array<int> & predicted_column,
                                int marker_column,
                                int & fatal)
...
static int 
determine_numbers_to_check (resizable_array<int> & numbers_to_check,
                            int records_read)
{
  ...
  if (percents_to_check.number_elements())
  {
    for (int i = 0; i < percents_to_check.number_elements(); i++)
    {
      ...
      int n = int(static_cast<float>(p * records_read) / 99.999);    // use a number < 100 to give a result just larger than the likely integer
      ...
      if (n > 1 && n <= records_read)
      {
        ...
        cerr << p << " percent of " << records_read << " is " << n << " which is ignored\n";
      else
...
int
Float_Comparator::operator() (const float & f1,
...
static int
compute_distribution_function (int number_records,
                               const float * v,
                               const float minval,
                               const float maxval,
                               int * d)
...
static int write_distribution_function (const char * prefix,
                                        int number_records,
                                        const int * d,
                                        std::ostream & output)
{
...
static int
do_compute_distribution_functions(int number_records,
...
static int 
initilize_enrichment (int number_records,
    		      int experimental_column,
                      const resizable_array_p<Predicted_Values> & zdata,
		      float * tmp1,
		      int which_predicted_set)
...
static int
compute_b_squared (int number_records,
                   int experimental_column,
                   const resizable_array_p<Predicted_Values> & zdata,
                   float * tmp1,
                   float * tmp2,
                ...
                IW_STL_Hash_Map_int & markers_present)
{
  ...
  if (verbose)
    cerr << "Across " << n << " records found " << markers_present.size() << " unique markers\n";
               ...
               resizable_array_p<Predicted_Values> & zdata)
{
  ...
  for (int i = 0; i < records_to_skip; i++)
  {
    ...
    {
      if (! establish_column_titles(buffer, experimental_column, predicted_column, marker_column, marker_column_name))
        return 0;
    }
  }
  ...
  while (input.next_record(buffer))
  {
    ...
    int fatal;
    if (! p->parse_buffer(buffer, experimental_column, predicted_column, marker_column, fatal))
    {
      delete p;
               ...
               resizable_array_p<Predicted_Values> & zdata)
{
  ...
  return read_the_data(input, experimental_column, predicted_column, marker_column, marker_column_name, zdata);
}

static int
echo_the_data (const resizable_array_p<Predicted_Values> & zdata,
               std::ostream & output)
{
...
static void
establish_range_of_experimental_data(int number_records,
                                     int experimental_column,
                                     const resizable_array_p<Predicted_Values> & zdata,
...
static double
compute_relative_error (float obs,
                        float pred)
{
  ...
  if (fabs(obs) < 1.0e-05 && fabs(pred) < 1.0e-05)    // both are very small - probably covered above
    return 0.0;
...
static double
compute_range_normal_relative_error (experimental_value_t obs,
                                     experimental_value_t pred,
                                     double obs_range)
{
...
static int
write_cmsr (const CMSR_Parameters & cmsrp,
...
static int
write_fold_data (const int predicted_column,
                 const resizable_array<float> & fold_difference,
                 const int * n_fold_difference, 
                 const int N,
                 const char star_or_plus,
                 std::ostream & output)
{
  write_something_identifying_the_column(predicted_column, output);
...
static void
compute_fold_difference_values_additive (const float obs,
                                const float pred,
                                const resizable_array<float> & fold_difference,
...
static void
compute_fold_difference_values_multiplicative (const float obs,
                                const float pred,
                                const resizable_array<float> & fold_difference,
                                int * n_fold_difference)
{
         ...
         std::ostream & output)
{
  ...
  if (truncate_predicted_values_to_experimental_range)
  {
    ...
    establish_range_of_experimental_data(number_records, experimental_column, zdata, lower_experimental_value, upper_experimental_value);
  }
...
// Do means and R2
  ...
  if (fold_difference_multiplicative.number_elements())
    n_fold_difference_multiplicative = new_int(fold_difference_multiplicative.number_elements());
...
//    tmp2[ndx] =     (obs - pred);
      ...
      if (fold_difference_additive.number_elements())
        compute_fold_difference_values_additive(obs, pred, fold_difference_additive, n_fold_difference_additive);
      if (fold_difference_multiplicative.number_elements())
        compute_fold_difference_values_multiplicative(obs, pred, fold_difference_multiplicative, n_fold_difference_multiplicative);
...
//  cerr << " v1 " << v1 << " v2 " << v2 << '\n';
    ...
    if (range_normal_relative_error)
      output << "Average Range Normal Relative Error " << static_cast<float>(rnre.average()) << '\n';
...
//  output << " R2 " << (sum1 / (p.variance() * o.variance()) / static_cast<double> (number_records)) << '\n';
  ...
  if (fold_difference_additive.number_elements())
    write_fold_data(predicted_column, fold_difference_additive, n_fold_difference_additive, number_records, '+', output);

  if (fold_difference_multiplicative.number_elements())
    write_fold_data(predicted_column, fold_difference_multiplicative, n_fold_difference_multiplicative, number_records, '*', output);

  int duplicate_predicted_values_present;
  if (predicted_column >= 0)
    duplicate_predicted_values_present = duplicate_predicted_values(zdata, which_predicted_set);
  else
                              ...
                              int experimental_column)
{
  ...
  if ("." == act)
  {
    cerr << "Missing value encountered in experimental file '" << buffer << "', impossible\n";
    return 0;
  }
...
template <typename T>
int
get_comma_separated_values (Command_Line & cl,
                            const char flag,
                            resizable_array<T> & v,
                            const float minval)
{
  ...
  for (int i = 0; cl.value(flag, f, i); ++i)
  {
    ...
    while (f.nextword(token, j, ','))
    {
      ...
      {
        cerr << "The -" << flag << " must be greater than " << minval << " '" << token << "' invalid\n";
        return 0;
      }
                               ...
                               double & discard_measured_values_below)
{
  ...
  if (verbose)
    cerr << "Keeping the " << fraction_to_keep << " most active values, implies an activity cutoff of " << discard_measured_values_below << " (range " << v[0] << " to " << v[ndx-1] << ")\n";

  return 1;
}

int
parse_digits (const const_IWSubstring & token,
              int istart,
              int & zresult)
{
  zresult = 0;
...
static int
parse_dash_p (const const_IWSubstring & p,
...
iwstats (int argc, char ** argv)
{
  ...
  if (cl.option_present('E'))
  {
    ...
    if (! read_id_activity_hash(e, id_activity_hash, experimental_column) || 0 == id_activity_hash.size())
    {
    ...
    is_descriptor_file = 1;
    experimental_column = 0;     // activity data has come from a separate file, don't go looking for it in the input file
  ...
  {
    if (! cl.value('c', values_needed_for_reporting_correlations) || values_needed_for_reporting_correlations < 3)
    {
      cerr << "The number of pairs needed for correlation compuations (-c) must be 3 or larger\n";
      usage(4);
    }

    if (verbose)
      cerr << "Correlations only done when at least " << values_needed_for_reporting_correlations << " data points available\n";
  }
  ...
  if (cl.option_present('p'))
  {
    ...
    {
      cerr << "Column " << experimental_column << " cannot be both predicted and experimental\n";
      return 8;
    }
  }
  ...
  {
    cerr << "Sorry, the no-report missing values option (-q) only makes sense with the -M option\n";
    usage(2);
  }
  ...
  {
    if (! cl.value('r', relative_error_threshold) || relative_error_threshold <= static_cast<float>(0.0))
    {
      cerr << "The max relative error (-r) option must be a non zero, positive floating point value\n";
      usage(2);
    }
  ...
  int n = (cl.option_present('n') > 0) + (cl.option_present('t') > 0) + (cl.option_present('P') > 0);
...
//  cerr << cl.option_present('n') << ' ' << cl.option_present('t') << ' ' << cl.option_present('P') << ' ' << n << '\n';
  ...
  if (cl.option_present('t'))
  {
    ...
    if (verbose)
      cerr << "Will compute Bsquared values for " << step << " sized sections of the data\n";
  }
  ...
  if (cl.option_present('b'))
  {
    ...
    {
      cerr << "The number of buckets for distribution studies (-b) must be a whole +ve number\n";
      usage(5);
    }

    if (verbose)
      cerr << "Will report distribution in " << number_distribution_buckets << " buckets\n";
  }
...
*/
  ...
  if (cl.option_present('U'))
  {
    ...
    {
      cerr << "The fraction of most active to retain (-U) option must be a valid fraction\n";
      usage(2);
    }
