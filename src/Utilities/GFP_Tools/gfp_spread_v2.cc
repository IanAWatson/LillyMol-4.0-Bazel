...
//cerr << "Pool ptr " << poolptr << ", pool size " << pool_size << endl;
  ...
  while (tdt.next(input))
  {
    ...
    {
      get_previously_computed_nearest_neighbour(tdt, pool[poolptr], previously_computed_nn_distance_tag);  // should really check for fatal errors
    }
    else if (previously_computed_distances.size())
    {
      if (! get_previously_computed_nearest_neighbour(pool[poolptr], previously_computed_distances))
        return 0;
    }
    else if (previous_computed_distance_column > 0)
    {
      if (! pool[poolptr].set_distance_to_previously_selected_from_column(previous_computed_distance_column))
      {
        cerr << "Cannot set previously selected distance based on column '" << pool[poolptr].id() << "'\n";
        return 0;
      }
    }
...
static int
do_squeeze_pool (int & istart, int & istop)
{
  int iptr = 0;
...
choose_largest_previously_computed_distance()
{
  ...
  if (rc < 0)
  {
    cerr << "Warning, none of " << pool_size << " items have previously computed distances!\n";
    rc = 0;
  }
...
// We need a heuristic about when to turn off the atom count window if being used

  int strikes = 0;

  int number_selected = 1;

  int istart = 0;     // first unselected point
  int istop = pool_size;    // last unselected point
...
//    cerr << number_selected << " distance " << i << " '" << pool[i].id() << "' is " << pool[i].distance() << endl;
...
//  cerr << "next " << ichoose << " '" << pool[ichoose].id() << "'\n";

    istart = firstunselected;
    istop = lastunselected + 1;
    ...
    if (verbose > 1)
      cerr << "Selected " << number_selected << " '" << fpsel.id() << "' (index " << ichoose << ") distance " << fpsel.distance() << " NSN '" << sid.id() << "'\n";
    ...
    int molecules_skipped_for_being_too_close = 0;
    ...
    if (verbose > 2)
      cerr << molecules_skipped_for_being_too_close << " molecules_skipped_for_being_too_close\n";

    if (! use_atom_count_window)
      ;
    else if (0 == molecules_skipped_for_being_too_close)
      ;
    else if (static_cast<double>(molecules_skipped_for_being_too_close) / static_cast<double>(pool_size - number_selected) > 0.8)
    {
      strikes++;
  ...
  if (cl.option_present('M'))
  {
    ...
    while (cl.value('M', m, i++))
    {
      ...
      else if (m.starts_with("squeeze="))
      {
        ...
        if (verbose)
          cerr << "Already selected molecules will be squeezed out every " << squeeze_pool << " selections\n";
      }
      else if (m.starts_with("ldist="))
      {
        m.remove_leading_chars(6);
        if (! m.numeric_value(longest_distance_recognised) || longest_distance_recognised <= 0.0f || longest_distance_recognised > 1.0f)
        {
          cerr << "The longest recognised distance directive 'ldist=' must be a valid distance\n";
          exit(2);
        }
...
// Each can have its own filter if the name starts with FILTER:
  ...
  if (cl.option_present('S'))
  {
    ...
    {
      cerr << "Cannot specify how to select first molecule if already selected molecules present(-A)\n";
      usage(3);
    }
    ...
    else if (s.starts_with("fff="))
    {
      ...
      if (! s.numeric_value(start_with_object_furthest_from_first) || start_with_object_furthest_from_first < 1)
      {
