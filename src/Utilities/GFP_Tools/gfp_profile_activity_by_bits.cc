                              ...
                              IWString_and_File_Descriptor & output)
{
  ...
  float * rtmp = new float[number_classes]; std::unique_ptr<float[]> free_rtmp(rtmp);

  Ndx_and_Ratio * nr = new Ndx_and_Ratio[nbits]; std::unique_ptr<Ndx_and_Ratio[]> free_nr(nr);
                           ...
                           IWString_and_File_Descriptor & output)
{
  ...
  Class_Membership * cm = new Class_Membership[nbits]; std::unique_ptr<Class_Membership[]> free_cm(cm);
                          ...
                          std::ostream & os)
{
  ...
  int * items_in_class = new_int(max_class_number - min_class_number + 1); std::unique_ptr<int[]> free_iic(items_in_class);
