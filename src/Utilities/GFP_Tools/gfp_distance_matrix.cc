...
/*
  By default the fast I/O option has the numbers nicely lined up. That may not be appropriate in all cases
*/
...
/*
  Our output buffer is an int too. When we get a result, just copy an element from int_sr[]
  into the appropriate place in output_buffer;
*/
...
usage (int rc)
{
  ...
  cerr << " -l               lower triangular matrix only\n";
  cerr << " -m               output as similarities rather than distances\n";
  cerr << " -s <size>        specify the number of molecules\n";
  cerr << " -f               fast I/O, repeat to squeeze multiple blanks\n";
  cerr << " -g <char>        inter-token output separator when doing C++ I/O\n";
  cerr << " -C <fname>       comparator file, compare against all other files\n";
  cerr << " -j               output multi-token names as separate columns (-C only)\n";
  cerr << " -e               small memory option, requires recomputation instead of storage\n";
  ...
  cerr << " -r <rpt>         report progress every <rpt> fingerprints processed\n";
...
static int
allocate_pool (IW_General_Fingerprint * & pool,
               int pool_size)
{
...
static int
do_squeeze_multiple_blanks_from_fast_io (const int * output_buffer,
                                         int jstop,
...
static void
write_identifier (const IWString & id,
                  IWString_and_File_Descriptor & output)
{
...
static void
write_line_of_distance_matrix (const int * output_buffer,
                               int jstop,
                               IWString_and_File_Descriptor & output)
{
  if (fast_io)
...
compute_the_distance (IW_General_Fingerprint & fpi,
                      IW_General_Fingerprint & fpj)
{
...
static int
distance_matrix (IW_General_Fingerprint * pool1,
                 int n1,
                 IW_General_Fingerprint * pool2,
                 int n2,
                 IWString_and_File_Descriptor & output)
{
//output.resize(40000);
  ...
  {
    cerr << "Size mismatch for identical fingerprint sets, " << n1 << " and " << n2 << endl;
    return 0;
  }
  ...
  for (int i = 0; i < n1; i++)
  {
    ...
    for (int j = 0; j < jstop; j++)
    {
      ...
      assert (int (100.0 * result + 0.499999) >= 0 && int (100.0 * result + 0.499999) <= 100);
      if (fast_io)
        output_buffer[j] = int_sr[int (100.0 * result + 0.499999)];
      else
        output << token_separator << result;
//    cerr << "Completed i = " << i << " j = " << j << endl;
    }
...
static int
all_ids_have_same_token_count (const IW_General_Fingerprint * pool, 
                               int n)
{
  assert (n > 0);
  ...
  for (int i = 1; i < n; i++)
  {
    ...
    cerr << "Found '" << pool[i].id() << "' at i = " << i << " expected " << nwords << " tokens\n";
    return 0;
  }
...
distance_matrix (int argc, char ** argv)
{
  ...
  if (cl.option_present('g'))
  {
    ...
    if (1 != g.length())
    {
      cerr << "Sorry, the inter-token separator must be a single character only, '" << g << "' not allowed\n";
      return 2;
    }
