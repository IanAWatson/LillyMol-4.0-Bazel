...
template <typename T>
int
WriteSelectedBrief(const T* pool,
                   int selected,
                   IWString_and_File_Descriptor& output) {
        ...
        IWString_and_File_Descriptor& output)
{
  ...
  if (choose_first_point_randomly) {
    ...
    s = std::max_element(pool, pool + pool_size, [](const F& fp1, const F& fp2) {
...
#ifdef DEBUG_SPREAD4
    cerr << "Selected " << s << " dist " << pool[s].distance() << " nexts_shared " << nexts_shared << '\n';
#endif

    if (nexts_shared < 0) {  // we are ignoring nsel, unlikely to be a problem...
      cerr << "nexts_shared negative\n";
      for (int i = 0; i < pool_size; ++i) {
        if (! pool[i].selected()) {
...
#pragma omp critical
    ...
    cerr << "Selected " << s << " dist " << distances[s] << " nexts_shared " << nexts_shared << '\n';

    if (nexts_shared < 0) {  // we are ignoring nsel, unlikely to be a problem...
      //for (int i = 0; i < pool_size; ++i) {
      //  cerr << i << " Selected " << selected[i] << " distance " << distances[i] << " nsn " << nearest_previously_selected[i] << '\n';
      //}
      return items_selected;
    }
    ...
    if (next_squeeze == items_selected) {
      //cerr << "Before squeeze s " << s << " pool_size " << pool_size << '\n';
      s = do_squeeze(pool, pool_size, s, selected, nearest_previously_selected, distances);
      //cerr << "After  squeeze s " << s << " pool_size " << pool_size << '\n';
      next_squeeze += squeeze;
    }
  }
...
#pragma omp critical
    ...
    if (nexts_shared < 0)  // we are ignoring nsel, unlikely to be a problem...
    {
      mark_all_remaining_items_selected(pool,
                                        pool_size,
                                        selected_item,
                                        items_selected + 1,
                                        selected,
                                        nearest_previously_selected,
                                        distances);
      return pool_size;
    }
...
static int
assign_scaling_factors_from_file(const char* fname, int col, const IWString* pcn, int verbose)
{
...
read_pool(iwstring_data_source& input, F* pool, int& pool_size)
{
  ...
  for (; tdt.next(input) && ndx < pool_size; ndx++) {
    ...
    if (previously_computed_nn_distance_tag.length()) {
      ...
      if (!get_previously_computed_nearest_neighbour(pcn[ndx],
                                                     previously_computed_distances,
                                                     distances[ndx]))
        return 0;
    }
    else if (previous_computed_distance_column > 0) {
      if (!get_previously_computed_from_name_column(pcn[ndx],
...
gfp_spread_standard(int argc, char** argv)
{
  ...
  if (cl.option_present('p')) {
    ...
    if (scaling_factor_file_name.length() && col.length()) {
      cerr << "Cannot specify both scaling factor file name (FILE=) and scaling factor column "
              "(COL=)\n";
      usage(1);
    }
  ...
  if (cl.option_present('t')) {
    if (!cl.value('t', stop_once_distance_drops_below) || stop_once_distance_drops_below < 0.0f ||
        stop_once_distance_drops_below >= 1.0f) {
      cerr << "The stop selection distance option (-t) must be a valid distance\n";
      usage(1);
    }

    if (verbose)
      cerr << "Will stop selection once distance drops below " << stop_once_distance_drops_below
           << '\n';
  }
  ...
  cerr << "scaling_factor_specified " << scaling_factor_specified << '\n';
  cerr.flush();
  if (scaling_factor_specified)
