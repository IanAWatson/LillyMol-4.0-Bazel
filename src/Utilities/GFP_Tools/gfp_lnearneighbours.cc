...
class NN_Object : public FP_and_Smiles, public Neighbour_List<similarity_type_t, FP_and_Smiles, FP_and_Smiles>
{
  private:
...
// private functions
  ...
  public:
    ...
    int search_status () const { return _search_status;}
    void set_search_status (int s) { _search_status = s;}
};
...
//cerr << "equal_weight_tanimoto_distance_measure " << equal_weight_tanimoto_distance_measure << " values " << IW_General_Fingerprint::equal_weight_tanimoto(rhs) << ' ' << IW_General_Fingerprint::tanimoto(rhs) << " btw " << rhs.id() << " and " << id() << endl;
  ...
  if (equal_weight_tanimoto_distance_measure)
    d = static_cast<similarity_type_t>(1.0) - IW_General_Fingerprint::equal_weight_tanimoto(rhs);
  else if (tversky_parameters.active())
    d = static_cast<similarity_type_t>(1.0) - IW_General_Fingerprint::tversky(rhs, tversky_parameters);
  else
    d = static_cast<similarity_type_t>(1.0) - IW_General_Fingerprint::tanimoto(rhs);

  if (d < static_cast<similarity_type_t>(0.0) || d > static_cast<similarity_type_t>(1.0))
  {
    cerr << "Bad news, invalid distance " << d << " between '" << _id << "' and '" << rhs.id() << "'\n";
...
//cerr << "nbrs " << number_neighbours() << " max d " << distance_of_furthest_neighbour() << " max " << _upper_distance_threshold << endl;
  ...
  if (min_number_neighbours > 0 && number_neighbours() > min_number_neighbours)
  {
    ...
    if (outOfRange)
        Neighbour_List<similarity_type_t, FP_and_Smiles, FP_and_Smiles>::shrink(min_number_neighbours);
  }
  return;
}
...
NN_Object::construct_from_tdt (IW_TDT & tdt, int & fatal)
{
  ...
  if (! u.numeric_value(_upper_distance_threshold) || _upper_distance_threshold < 0.0 || _upper_distance_threshold > 1.0)
  {
...
nearneighbours_sorted_pool (FP_and_Smiles & fp)
{
  ...
  {
    cerr << "Supposedly sorted haystack not sorted, previous " << previous_natoms << " now " << n << " atoms, " << fingerprints_processed << " fingerprints read, '" << fp.id() << "'\n";
    abort();
  }
  ...
  for (int i = 0; i < pool_size; i++)
  {
    ...
    if (can_be_compared(pi, fp))
    {
      if (verbose > 1 && SEARCHING_ACTIVE != pi.search_status())
        cerr << "After " << fingerprints_processed << " fingerprints, pool item " << i << " (" << pi.natoms() << ") activated (" << fp.natoms() << ") " << number_active_fingerprints << " active fingerprints\n";
    ...
    else if (SEARCHING_ACTIVE == pi.search_status())   // was active, now turn off
    {
      ...
      if (verbose > 1)
        cerr << "After " << fingerprints_processed << " fingerprints, pool item " << i << " (" << pi.natoms() << ") turned off, (" << fp.natoms() << ") " << number_active_fingerprints << " remaining\n";
...
static int
compute_neighbours_stored (const NN_Object * pool,
                           int pool_size)
{
  int rc = 0;
  for (int i = 0; i < pool_size; i++)
...
{
  Command_Line cl (argc, argv, "vs:n:p:I:i:e:t:T:r:F:P:W:Q:V:hB:K:D:N:m:g:E:d");
...
// UPPER_DISTANCE_THRESHOLD_TAG is set. Also _upper_distance_threshold is set in the constructor
  ...
  if (cl.option_present('T'))
  {
    ...
    while (cl.value('T', t, i++))
    {
      if (t.starts_with("TAG="))
      {
        ...
        if (verbose)
          cerr << "The upper distance threshold for each molecule in the '" << upper_distance_threshold_tag << "' tag\n";
  ...
  {
    cerr << "This version only works with entropy weighted fingerprints, must use -E option\n";
    usage(3);
  }
  else
  {
...
// that the pool is built first
  ...
  if (cl.option_present('B'))
  {
    ...
    while (cl.value('B', b, i++))
    {
      ...
      else if ("help" == b)
      {
        ...
        cerr << " -B nbrtag=<tag> write the number of neighbours for each target in <tag>\n";
        ...
        cerr << " -B maxdistanceinclusive    distnaces up to and including the max distance are considered matches\n";
        return 0;
      }
      else
      {
  ...
  {
    if (! cl.value('g', must_be_larger_than) || must_be_larger_than < 0.0 || must_be_larger_than >= 1.0)
    {
      cerr << "The abandon distance computation option (-g) value must be a valid distance\n";
      usage(4);
    }
    pool_is_sorted = 1;

    if (verbose)
      cerr << "Assuming pool sorted, each similarity component must be at least " << must_be_larger_than << endl;
  ...
  if (cl.option_present('p'))
  {
    ...
    if (verbose && upper_distance_threshold_tag.length())
      cerr << items_with_max_distance_threshold_specified << " of " << pool_size << " fingerprints had upper distance threshold individually specified\n";
  }
...
//  cout << "Object " << i << " '" << pool[i].id () << "'\n";
    ...
    if (sort_neighbour_list_at_end)
      pool[i].sort_neighbour_list();

    if (min_number_neighbours > 0 && upper_distance_threshold <= 1.0)
      pool[i].remove_distant_neighbours(min_number_neighbours, upper_distance_threshold);
  ...
  if (verbose)
  {
    ...
    if (pool_is_sorted)
      cerr << similarity_computations_performed << " similarity computations done on sorted pool\n";
  }
