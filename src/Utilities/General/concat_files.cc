...
//  private functions
    ...
    int _append_everything_but_identifier_column(const_IWSubstring & buffer, IWString & output_buffer) const;

  public:
    AFile();

    int active() const { return _fname.length();}

    const IWString & fname() const { return _fname;}

    void set_file_name (const IWString & f) { _fname = f;}

    void set_identifier_column (int c) { _identifier_column = c;}
    void set_input_separator (char c) { _input_separator = c;}
    
    int initialise (const char *);

    int columns_in_file() const { return _columns_in_file;}
    int records_in_file() const { return _records_in_file;}
    int duplicate_identifiers() const { return _duplicate_identifiers;}

    const IWString & header() const { return _header;}
    const IWString & filename() const { return _fname;}
    const IWString & missing_values() const { return _missing_values;}

    int   missing_records() const { return _missing_records;}
    int & missing_records()       { return _missing_records;}
    ...
    int check_for_duplicate_descriptor_names (IWString_STL_Hash_Set & descriptor_names_encountered) const;
};
...
static int check_for_duplicate_descriptor_names = 0;
...
static int
preprocess_to_identifier(const_IWSubstring & buffer, 
                         const int identifier_column,
                         const int quot,
                         const char sep)
{
...
//  cerr << "From '" << buffer << "' col " << identifier_column << " get '" << token << "'\n";
  ...
  if (0 == buffer.length())
  {
    cerr << "preprocess_to_identifier:zero length identifier, column " << identifier_column << " sep '" << sep << "'\n";
    return 0;
  }
...
static int
recognise_file_qualifiers(const char * s,
                          char & sep,
                          int & col,
                          int & quot,
                          IWString & fname)
...
AFile::initialise(const char * fname)
{
  ...
  if (! recognise_file_qualifiers(fname, _input_separator, _identifier_column, _quoted_fields, myfname))
    return 0;
...
//cerr << "AFile::initialise:from '" << fname << "' column " << _identifier_column << " sep '" << _input_separator << "'\n";
  ...
  while (iwstring_data_source::next_record(buffer))
  {
    ...
    if (0 == buffer.length())
    {
      cerr << "Blank line at line " << iwstring_data_source::lines_read() << " in '" << fname << "'\n";
    ...
    if (! preprocess_to_identifier(buffer, _identifier_column, _quoted_fields, _input_separator))
    {
      cerr << "Cannot extract identifier from '" << buffer << "' col " << (_identifier_column+1) << endl;
      return 0;
    }
    ...
    else
    {
      ...
      if (verbose < 2)
        ...
        cerr << "Identifier '" << buffer << "' found at " << offset << ", size " << size() << endl;
      else
...
/*
  Append columns 1-(col-1) to output_buffer, then columns (col+1)-
*/
                                                ...
                                                IWString & output_buffer) const
{
  ...
  for (int c = 0; iwt.next_token(token); ++c)
  {
    if (c == _identifier_column)
      continue;
...
template <typename T>
int
append_possibly_translated(const T & s,
                           const char inp_sep,
                           const char out_sep,
                           const int quot,
                           IWString & output)
...
{
  return append_possibly_translated(_header, _input_separator, output_separator, _quoted_fields, output);

  return 1;
}

int
AFile::echo (const IWString & id,
...
// Make sure we don't write the identifier again. If it is in the first column, that's
// very easy. Otherwise we need to work harder

  if (0 == _identifier_column)     // the most common case. 
  {
    buffer.remove_leading_words(1, _input_separator);     // don't write the identifier again
    output_buffer << output_separator;
    append_possibly_translated(buffer, _input_separator, output_separator, _quoted_fields, output_buffer);
  }
  else
...
static int
write_unwritten_identifier(const const_IWSubstring & id,
...
int
AFile::check_for_duplicate_descriptor_names(IWString_STL_Hash_Set & descriptor_names_encountered) const
{
  if (all_descriptors_unique(_header, descriptor_names_encountered, _input_separator))
    return 1;

  cerr << "AFile::check_for_duplicate_descriptor_names:duplicate descriptor found in '" << _fname << "'\n";
  return 0;
}
...
static int
write_unprocessed_identifiers_to_K_file_if_requested(AFile * files, const int nfiles,
                        std::ofstream & stream_for_identifiers_not_written)
{
...
static int
write_identifiers_not_written(AFile * files,
                              const int nfiles,
...
static int
do_all_identifiers(const IWString & missing,
                   int & records_written,
                   AFile * files,
                   const int nfiles,
                   const int z,
...
static int
do_all_identifiers(int columns_so_far,
                   int & records_written,
                   AFile * files,
                   const int nfiles,
                   int z,
                   ...
                   IWString_and_File_Descriptor & output)
{
  for (int i= 1; i < nfiles; i++)
  {
    ...
    if (0 == f.size())      // all identifiers in this file written
      ;
    else if (! do_all_identifiers(columns_so_far, records_written, files, nfiles, i, output))
      return 0;
...
static int
concat_files(iwstring_data_source & input,
             ...
             IWString_and_File_Descriptor & output)
{
  ...
  if (check_for_duplicate_descriptor_names)
  {
    IWString_STL_Hash_Set descriptor_names_encountered;
    ...
    for (int i = 1; i < nfiles; ++i)
    {
      if (! files[i].check_for_duplicate_descriptor_names(descriptor_names_encountered))
      {
        return 0;
      }
    }
...
//  buffer.strip_leading_blanks();
    ...
    {
      if (stream_for_identifiers_not_written.rdbuf()->is_open())
        write_identifiers_not_written(files, nfiles, id, stream_for_identifiers_not_written);
  ...
  if (only_write_records_when_all_files_have_data)
    return write_unprocessed_identifiers_to_K_file_if_requested(files, nfiles, stream_for_identifiers_not_written);
              ...
              IWString_and_File_Descriptor output)
{
  ...
  if (! recognise_file_qualifiers(fname, input_separator, identifier_column, quot, myfname))
    return 0;
  ...
  files[0].set_file_name(fname);     // files[0] never calls its initialise() method, so we need to tell it its file name

  return concat_files(input, input_separator, identifier_column, quot, files, nfiles, output);
}

static int
concat_files (int argc, char ** argv)
{
  Command_Line cl(argc, argv, "vuM:adgzc:IK:nksX:qQbi:o:f");
  ...
  {
    check_for_duplicate_descriptor_names = 1;
  ...
  {
    cerr << "The do all identifiers (-a) and only do identifiers with complete data (-I) options are mutually exclusive\n";
    usage(6);
  }
  ...
  if (cl.option_present('X'))
  {
    ...
    if (verbose)
      cerr << "Files matching '" << suffix_exclusion_list->pattern() << "' will be skipped\n";
    ...
    {
      cerr << "Sorry, only " << valid_files_on_command_line << " files can be processed\n";
      return 4;
    }

    if (verbose)
      cerr << files_excluded << " files excluded, " << valid_files_on_command_line << " files remain\n";
  }

  missing_dataitem = new_int(nfiles); std::unique_ptr<int> free_missing_dataitem(missing_dataitem);
...
// We will echo the first file and append the others. Establish offsets in the others
  ...
  for (int i = 1; i < nfiles; i++)
  {
    ...
    if (verbose)
    {
      cerr << "File '" << cl[i] << "' contains " << files[i].records_in_file() << " records of " << files[i].columns_in_file() << " columns each";
  ...
  if (verbose)
  {
    ...
    if (items_missing)
    {
      for (int i = 1; i < nfiles; i++)
      {
        ...
        cerr << "File " << i << " '" << files[i].filename() << "' had " << files[i].missing_records() << " missing records\n";
      }
    }
    ...
    if (items_missing)
    {
      for (int i = 0; i < nfiles; i++)
      {
        ...
        cerr << missing_dataitem[i] << " dataitems missing from file " << i << ", '" << files[i].fname() << "'\n";
      }
    }
    else
