...
static std::unique_ptr<RE2> rx;

static std::unique_ptr<RE2> end_of_record_rx;
...
class streampos_hash_fn : public std::hash<off_t>
{
  private:
  public:
                   ...
                   int & rx_matched)
{
  ...
  while (input.next_record(buffer))
  {
    ...
    if (rx && 0 == rx_matched && RE2::PartialMatch(tmp, *rx))
      rx_matched = 1;

    if (! end_of_record_rx)    // one line per record
      return lines_this_record;

    if (RE2::PartialMatch(tmp, *end_of_record_rx))
      return lines_this_record;
  }
...
random_records (int argc, char ** argv)
{
  ...
  if (cl.option_present('R'))
  {
    ...
    if (verbose)
      cerr << "Will only retrieve records matching '" << rx->pattern() << "'\n";
  }
  ...
  if (cl.option_present('E'))
  {
    ...
    re2::StringPiece tmp(r.data(), r.length());
    end_of_record_rx.reset(new RE2(tmp));
    if (! end_of_record_rx->ok())
    {
    ...
    if (verbose)
      cerr << "End of record regular expression'" << end_of_record_rx->pattern() << "'\n";
  }
