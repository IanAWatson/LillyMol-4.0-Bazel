...
Stream_for_Columns::Stream_for_Columns (int nc, const char * fname) :
         _tokens (nc)
{
...
static int
process_to_streams (const const_IWSubstring & buffer,
                    int ntokens)
{
...
//cerr << "sizeof (off_t) " << sizeof (off_t) << " sizeof (streampos) " << sizeof (streampos) << endl;
  ...
  if (!showDelimiterOptions)
    cerr << "  -d <delim>        delimiter is any character, or a token word(e.g. 'tab'). see '-d help' for a list (default ' ')\n";
  ...
  if (showDelimiterOptions)
  {
    cerr << "  -d <delim>        delimiter is any character, or one of these token words:\n";
...
static int
header_record_has_only_different_token_count (const extending_resizable_array<int> & token_counts,
                               const int tokens_on_first_record)
{
  ...
  for (int i = 1; i < nt; i++)
  {
    ...
    {
      if (1 != token_counts[i])   // if an R file, there must be a single header record with that token count
        return 0;
...
static void
determine_per_token_lengths(const const_IWSubstring & buffer,
                            const char token_delimiter)
{
...
tcount(iwstring_data_source & input)
{
  ...
  while (input.next_record(buffer))
  {
    ...
    if (verbose > 2)
      cerr << "Record " << input.lines_read() << " had " << tokens << " words and " << buffer.length() << " characters\n";
...
tcount (const char * fname)
{
  ...
  if (verbose)
  {
    cerr << " records between " << record_length.minval() << " and " << record_length.maxval();
...
tcount (int argc, char ** argv)
{
  ...
  if (cl.option_present('d'))
  {
    ...
    {
      cerr << "Sorry, the delimiter character can be one character only, or one of a list of tokens.  see -d help for details\n";
      usage(13);
    }
...
//    consecutive_delimiters_mean_empty_words_between = 1;
  ...
  if (cl.option_present('n'))
  {
    ...
    if (verbose)
      cerr << "Will read only the first " << records_to_process << " records from each file\n";
  }
  ...
  if (cl.option_present('L'))
  {
    ...
    {
      cerr << "The number of columns for which you want length stats (-L) must be a whole +ve number\n";
      usage(1);
    }
  ...
  if (cl.option_present('r'))
  {
    ...
    if (verbose)
      cerr << "Will report all instances where token count is different from previous record\n";
  }
  ...
  if (cl.option_present('U'))
  {
    ...
    new_fstream_unreported.open(new_fname_unreported.null_terminated_chars(), std::ios::out);
