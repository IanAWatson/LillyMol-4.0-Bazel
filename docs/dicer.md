...
can be broken. Generally, ring and multiple bonds will be broken, although the 
...
actual number of fragments generated will exceed the limit.

For example, CHEMBL4524052 contains 636 atoms and 360 rotatable bonds. If dicer
is allowed to process this with no constraints, after 5 minutes of processing
it has generated qq fragments.
...
breakages may be a reasonable comparomise. For example our large Chembl molecule
...
(see below) to `-m 5 -M 15` only 21k fragments are generated, and run time is 0.7 seconds.
...
matched atoms in the query define the bond.
...
Experience tells us that differentiating I, Cl, and Br is seldom useful, and
the -T option enables element transformations to do this. Add `-T I=Cl -T Br=Cl`
to translate all heavy halogens to `Cl`.
...
not produced.
#### bscb
Allow Carbon-Carbon single bonds to break. By default, these are not broken.
#### xsub
