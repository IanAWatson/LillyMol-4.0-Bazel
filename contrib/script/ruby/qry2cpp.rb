...
# Remains to be seen if I ever get it working, there are some
...
def add_ring_specifiers(qry, ndx, src)
  ...
  qry.ring_specifier.each_with_index do |r, i|
    ...
    src << "  Molecule& m = *target.molecule();"
    src << "  (void) m.natoms();  // avoid complaints about unused variable, depends on what is queried"
...
def add_specifier_matcher(qry, uid, src)
  ...
  if ! qry.has_aromatic?
    ...
    src << "  if (! atom.is_aromatic()) return 0;"
  else
    src << "  if (atom.is_aromatic()) return 0;"
  end
...
def add_query_atom(qry, ndx1, ndx2, src)
  ...
  # First write a function for each of the specifiers.
   ...
   if qry.atom_properties.size == 1
     src << "  const int res = #{specifier_fns[0]}(target[zatom]);"
   ...
   specifier_fns.each_with_index do |fn, i|
     ...
     src << "    const int res = #{fn}(target[zatom]);"
...
def write_closing(qry, output)
  src = []
  src << "int"
  src << "SubstructureSearch(Molecule& m) {"
  src << "  if (m.empty()) return 0;"
  src << "  const int matoms = m.natoms();"
  src << "  std::unique_ptr<int[]> matched(new_int(matoms));"
  if qry.query.empty?
  elsif qry.query.size == 1
  else
  end
  src << "  return 1;"
  src << "}"
