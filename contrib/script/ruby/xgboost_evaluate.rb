# Evaluate a xgboost model built with svmfp_make
...
def usage(retcod)
  $stderr << "Evaluate a xgboost model build with svmfp_make\n"
...
class Model
  ...
  def initialize(mdir)
    ...
    @data = GfpModel::XGBoostModel.decode(File.read(@model_data_fname))
    # $stderr << @data
  end
...
models.each do |model|
  tmpsvml = Tempfile.new('xgboost.svml')
  ...
  tmplog = Tempfile.new('xgboost.log')
  tmpresult = Tempfile.new('xgboost.txt')
  cmd = "#{evaluate} #{mdir}/#{model.conf_file} task=pred model_in=#{mdir}/#{model.model_file} " \
        "test:data=#{tmpsvml.path}?format=libsvm name_pred=#{tmpresult.path}" 
